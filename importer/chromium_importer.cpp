...
bool ChromiumImporter::ReadAndParseSignons(const base::FilePath& sqlite_file,
  std::vector<autofill::PasswordForm>* forms) {
  sql::Connection db;
...
bool ChromiumImporter::ReadAndParseHistory(const base::FilePath& sqlite_file,
  ...
  const char query2[] = "SELECT url, title, visit_count, hidden,"
              " typed_count, case when last_visit_time = 0 then 1 "
              " else last_visit_time end as last_visit_time FROM urls";
