...
EVP_BytesToKey(EVP_des_ede3_cbc(), EVP_sha1(), record_salt, (const unsigned char *) password, sizeof(password)-1, 6300, key,iv);
*/
...
<record len><saltlen><salt><datalen><data> Record: GUID?
  data: little-endian UTF16 text, null terminated
<record len><saltlen><salt><datalen><data> Record: expiration date? 1970-01-01T00:00:00Z -> time_t, 0: never?
  data: little-endian UTF16 text, null terminated
...
unsigned char OperaObfuscationPass[] = { 0x83, 0x7D, 0xFC, 0x0F, 0x8E, 0xB3, 0xE8, 0x69, 0x73, 0xAF, 0xFF };

char temporary_security_password[] = "foobar1.";

#define TAG_MSB 0x80000000

bool DecryptMasterPasswordKeys(const std::string &password, const std::string &salt, const std::string &in_data, std::string &out_data)
{
	base::MD5Digest res1;
	...
	base::MD5Init(&ctx);
	base::MD5Update(&ctx, base::StringPiece((const char *) res1.a, sizeof(res1.a)));
	base::MD5Update(&ctx, base::StringPiece(password));
	base::MD5Update(&ctx, salt);
	base::MD5Final(&res1, &ctx);
	...
	scoped_ptr<crypto::SymmetricKey> dec_key(
		crypto::SymmetricKey::Import(crypto::SymmetricKey::DES_EDE3, key, sizeof(key)));
...
bool ValidatePasswordBlock(const std::string &password, const std::string &salt, const std::string &validation_string, const std::string &in_data)
{
	if(salt.length() != 8)
		return false;
...
bool WandReadUint32(std::string::iterator &buffer, std::string::iterator &buffer_end, uint32 &result)
{
	...
	for(i =4; i>0 && buffer < buffer_end; i--, buffer++){
		...
		temp_result = (temp_result << 8) | uint32(b1);
	}
...
bool WandReadUintX(std::string::iterator &buffer, std::string::iterator &buffer_end, uint32 int_len, uint32 &result, bool check_msb = false)
{
	uint32 temp_result = 0;
	int i;
...
bool WandReadBuffer(std::string::iterator &buffer, std::string::iterator &buffer_end, unsigned char *target, uint32 len)
{
...
bool WandReadVector(std::string::iterator &buffer, std::string::iterator &buffer_end, binary_string &target, uint32 len)
{
	target.resize(len);
...
bool WandReadTagLenDataVector(std::string::iterator &buffer, std::string::iterator &buffer_end, uint32 tag_len, uint32 len_len, uint32 &tag, binary_string &target)
{
	target.resize(0);
	tag = 0;
...
bool WandReadEncryptedField(std::string::iterator &buffer, std::string::iterator &buffer_end, base::string16 &result, binary_string *master_password = NULL)
{
	uint32 len=0;
	uint32 iv_len=0;
	uint32 data_len=0;
	...
	if (master_password && master_password->length())
	{
		if(!DecryptMasterPasswordKeys(*master_password, iv, in_data, out_data) || 
			!ValidatePasswordBlock(*master_password, iv,"Opera Email Password Verification", out_data))
			return false;
	}
	else
	{
		...
		base::MD5Init(&ctx);
		base::MD5Update(&ctx, base::StringPiece((char *) OperaObfuscationPass, sizeof(OperaObfuscationPass)));
		...
		scoped_ptr<crypto::SymmetricKey> dec_key(crypto::SymmetricKey::Import(crypto::SymmetricKey::DES_EDE3, key));

		crypto::Encryptor decryptor;
		if(!decryptor.Init(dec_key.get(), crypto::Encryptor::CBC, base::StringPiece((char *) res2.a+8, 8)))
			return true;
	...
	for (binary_string::iterator it = out_data.begin(); it < out_data.end();)
	{
		...
		it++;
		uint16 b1 = uint16(c01);
		if (it >= out_data.end())
			return false;
		unsigned char c02 = *it;
		it++;
		uint16 b2 = uint16(c02);
...
bool WandReadEncryptedNameAndField(std::string::iterator &buffer, std::string::iterator &buffer_end, base::string16 &name, base::string16 &value, bool &is_password, binary_string *master_password = NULL)
{
	is_password = false;
...
bool OperaImporter::ImportWand_ReadEntryHTML(std::string::iterator &buffer, std::string::iterator &buffer_end, std::vector<autofill::PasswordForm> &passwords, bool ignore_entry)
{
	base::string16 guid;
	...
	{
		if(!WandReadEncryptedNameAndField(buffer, buffer_end, 
				fields[i].fieldname, 
				fields[i].fieldvalue, 
				fields[i].is_password,
				&master_password_block))
			return false;
	...
	for (uint32 i=0; i< field_count; i++)
		if (i != first_field && i != first_pass && !fields[first_pass].is_password)
			password.other_possible_usernames.push_back(fields[first_pass].fieldvalue);
...
bool OperaImporter::ImportWand_ReadEntryAuth(std::string::iterator &buffer, std::string::iterator &buffer_end, std::vector<autofill::PasswordForm> &passwords, bool ignore_entry)
{
	base::string16 guid;
	base::string16 date_used;
	base::string16 url;
...
bool OperaImporter::GetMasterPasswordInfo()
{
	...
	if(!WandReadUint32(sec_buffer, sec_buffer_end, 	dummy) || dummy != 0x00001000)
		return false;
	if(!WandReadUint32(sec_buffer, sec_buffer_end, 	dummy) || dummy < 0x05050000 || dummy >= 0x05060000 )
		return false;
	...
	if (!WandReadTagLenDataVector(sec_buffer, sec_buffer_end, tag_len, len_len, tag, master_sec_block) || tag != 0x04)
		return false;
	...
	{
		binary_string temp;
		if (!WandReadTagLenDataVector(sec_buffer, sec_buffer_end, tag_len, len_len, tag, temp))
			return false;
		switch(tag)
		{
		case 0x50:
	...
	master_password8 = base::UTF16ToUTF8(master_password);
	binary_string master_sec_decrypted;

	if(!DecryptMasterPasswordKeys(master_password8, master_sec_salt, master_sec_encrypted, master_sec_decrypted) || 
		!ValidatePasswordBlock(master_password8, master_sec_salt,"Opera SSL Password Verification", master_sec_decrypted))
		return false;

	master_password_block.assign(master_password8);
	master_password_block.append(master_sec_decrypted);
...
void OperaImporter::ImportWand()
{
	...
	if (master_password_required && !GetMasterPasswordInfo())
		return;
	...
	uint32 masterpass_used=0;
	if (!WandReadUint32(wand_buffer, wand_buffer_end, masterpass_used))
		return;
	if (masterpass_used != 0 && master_password_block.length() > 0 && !GetMasterPasswordInfo())
		return;
	...
	if (!cancelled())
	{
		for(std::vector<autofill::PasswordForm>::iterator it = passwords.begin(); it < passwords.end(); it++)
