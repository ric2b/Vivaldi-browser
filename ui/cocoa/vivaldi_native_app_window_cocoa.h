...
#ifndef UI_VIVALDI_NATIVE_APP_WINDOW_COCOA_H_
#define UI_VIVALDI_NATIVE_APP_WINDOW_COCOA_H_
...
@interface VivaldiNativeAppWindowController : NSWindowController<NSWindowDelegate> {
 @private
  VivaldiNativeAppWindowCocoa* appWindow_;  // Weak; owns self.
  base::scoped_nsobject<NSView> titlebar_background_view_;
}

@property(assign, nonatomic) VivaldiNativeAppWindowCocoa* appWindow;

// VivaldiNativeAppWindowController will retain this view and call
...
class VivaldiNativeAppWindowCocoa : public VivaldiNativeAppWindow {
 public:
  ...
  // Called to start the close down sequence, run beforeunload handlers, etc.
 ...
 private:
  ~VivaldiNativeAppWindowCocoa() override;

  VivaldiAppNSWindow* window() const;
  ...
  VivaldiBrowserWindow* app_window_;  // weak - AppWindow owns NativeAppWindow.
  ...
  base::scoped_nsobject<VivaldiNativeAppWindowController> window_controller_;
...
#endif  // UI_VIVALDI_NATIVE_APP_WINDOW_COCOA_H_
