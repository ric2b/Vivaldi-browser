...
// pin. Because bookmark app shortcuts are shortcuts to Vivaldi (plus a few
// command line parameters) Windows ends up using the Vivaldi icon specified in
// vivaldi.exe.manifest instead of the site's icon stored inside the shortcut.
//
// The vivaldi_proxy.exe binary workaround "fixes" this by having bookmark app
// shortcuts target vivaldi_proxy.exe instead of vivaldi.exe such that Windows
