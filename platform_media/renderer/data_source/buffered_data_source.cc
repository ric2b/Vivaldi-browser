...
BufferedResourceLoader* BufferedDataSource::CreateResourceLoader(
  ...
  BufferedResourceLoader::DeferStrategy strategy = preload_ == MultibufferDataSource::METADATA ?
...
void BufferedDataSource::SetPreload(MultibufferDataSource::Preload preload) {
  DCHECK(render_task_runner_->BelongsToCurrentThread());
  preload_ = preload;
}

void BufferedDataSource::SetBufferingStrategy(
    MultibufferDataSource::BufferingStrategy buffering_strategy) {
  DCHECK(render_task_runner_->BelongsToCurrentThread());
...
const std::string& BufferedDataSource::mime_type() const {
  return mime_type_;
}

void BufferedDataSource::OnBufferingHaveEnough(bool always_cancel) {
  DCHECK(render_task_runner_->BelongsToCurrentThread());
  if (loader_ && (always_cancel || (preload_ == MultibufferDataSource::METADATA &&
...
void BufferedDataSource::UpdateDeferStrategy() {
  ...
  bool aggressive = (buffering_strategy_ == MultibufferDataSource::BUFFERING_STRATEGY_AGGRESSIVE);
