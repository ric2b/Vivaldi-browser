...
struct PrefsMapping kPrefsValues[] {
  // clang-format off
    ...
    // Control native window decoration
    { vivaldiprefs::kVivaldiUseNativeWindowDecoration, booleanPreftype },
...
void VivaldiSettingsApiNotificationFactory::RegisterProfilePrefs(
  ...
  registry->RegisterBooleanPref(
      vivaldiprefs::kVivaldiUseNativeWindowDecoration, false,
      user_prefs::PrefRegistrySyncable::SYNCABLE_PREF);
...
ContentSetting
ConvertToContentSetting(vivaldi::settings::ContentSettingEnum setting) {
  switch (setting) {
...
ContentSettingsType
ConvertToContentSettingsType(vivaldi::settings::ContentSettingsTypeEnum type) {
  switch (type) {
