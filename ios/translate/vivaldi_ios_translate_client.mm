...
VivaldiIOSTranslateClient::VivaldiIOSTranslateClient(web::WebState* web_state)
...
VivaldiIOSTranslateClient::~VivaldiIOSTranslateClient() {
  DCHECK(!web_state_);
}

// static
std::unique_ptr<translate::TranslatePrefs>
VivaldiIOSTranslateClient::CreateTranslatePrefs(PrefService* prefs) {
  return std::unique_ptr<translate::TranslatePrefs>(
      new translate::TranslatePrefs(prefs));
}

translate::TranslateManager* VivaldiIOSTranslateClient::GetTranslateManager() {
  return translate_manager_.get();
}

/* static */
std::string& VivaldiIOSTranslateClient::GetTranslateScript() {
...
void VivaldiIOSTranslateClient::LoadTranslationScript() {
  if (VivaldiIOSTranslateClient::GetTranslateScript().empty()) {
    ...
    DCHECK(!script.empty());
    VivaldiIOSTranslateClient::GetTranslateScript() = ReplaceServerUrl(script);
  }
}
...
bool VivaldiIOSTranslateClient::ShowTranslateUI(
...
translate::IOSTranslateDriver* VivaldiIOSTranslateClient::GetTranslateDriver() {
  return &translate_driver_;
}

PrefService* VivaldiIOSTranslateClient::GetPrefs() {
  DCHECK(web_state_);
  ProfileIOS* profile =
...
std::unique_ptr<translate::TranslatePrefs>
VivaldiIOSTranslateClient::GetTranslatePrefs() {
  DCHECK(web_state_);
  ProfileIOS* profile =
      ProfileIOS::FromBrowserState(web_state_->GetBrowserState());
...
void VivaldiIOSTranslateClient::DidFinishNavigation(
    web::WebState* web_state,
    web::NavigationContext* navigation_context) {
...
void VivaldiIOSTranslateClient::WasShown(web::WebState* web_state) {
  if (translate_metrics_logger_) {
    translate_metrics_logger_->OnForegroundChange(true);
  }
}

void VivaldiIOSTranslateClient::WasHidden(web::WebState* web_state) {
  if (translate_metrics_logger_) {
    translate_metrics_logger_->OnForegroundChange(false);
  }
}

void VivaldiIOSTranslateClient::WebStateDestroyed(web::WebState* web_state) {
...
void VivaldiIOSTranslateClient::DidPageLoadComplete() {
  if (translate_metrics_logger_) {
