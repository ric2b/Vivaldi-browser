...
@interface NoteEditViewController () <NoteFolderViewControllerDelegate,
...
- (void)viewDidLoad {
  ...
  self.view.accessibilityIdentifier = kNoteEditViewContainerIdentifier;
...
- (void)updateUIFromNote {
  // If there is no current note, don't update.
  if (!self.note)
    return;
...
- (void)textFieldLongDidBeginEditing:(UITextField*)textField {
  //textField.textColor = [NoteTextFieldCell textColorForEditing:YES];
}

- (void)textFieldLongDidEndEditing:(UITextField*)textField {
 // textField.textColor = [NoteTextFieldCell textColorForEditing:NO];
}
...
- (NSArray*)keyCommands {
  __weak NoteEditViewController* weakSelf = self;
