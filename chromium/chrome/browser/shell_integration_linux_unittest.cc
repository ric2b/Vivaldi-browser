...
TEST(ShellIntegrationTest, GetExistingShortcutLocations) {
  ...
  const char kTemplateFilename[] = "vivaldi-test_extension-Profile_1.desktop";
...
TEST(ShellIntegrationTest, GetExtensionShortcutFilename) {
  ...
  EXPECT_EQ(base::FilePath("vivaldi-extensionid-Profile_Name_.desktop"),
            GetExtensionShortcutFilename(kProfilePath, kExtensionId));
}

TEST(ShellIntegrationTest, GetExistingProfileShortcutFilenames) {
  base::FilePath kProfilePath("a/b/c/Profile Name?");
  const char kApp1Filename[] = "vivaldi-extension1-Profile_Name_.desktop";
  const char kApp2Filename[] = "vivaldi-extension2-Profile_Name_.desktop";
  const char kUnrelatedAppFilename[] = "vivaldi-extension-Other_Profile.desktop";
