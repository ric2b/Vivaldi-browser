...
void DeleteProfileHelper::OnNewActiveProfileInitialized(
  ...
  // Vivaldi: Due to profile validation, when deleting profile while the only
  // other profile is not valid, called from profile_manager.cc
  // OnProfileCreationFinished the profile actually can be null.
  if (!loaded_profile) {
