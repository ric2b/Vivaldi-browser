...
void ChromeMetricsServicesManagerClient::UpdateRunningServices(
    bool may_record,
    bool may_upload) {
  // Vivaldi: Do not override value set by us in
  // components/crashreport/crashreport_observer.cc
  if (vivaldi::IsVivaldiRunning()) {
    const auto enable = vivaldi::IsVivaldiCrashReportingEnabled();
    install_static::SetCollectStatsInSample(enable);
    SetUploadConsent_ExportThunk(enable);
    return;
  }
