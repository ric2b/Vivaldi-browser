...
std::vector<content::WebContents*> PersistentTabRestoreService::RestorePreviousSession(
    TabRestoreServiceDelegate* delegate,
  ...
  for (TabRestoreService::Entries::iterator it = previous_entries.begin();
    ...
    std::vector<content::WebContents*> entryContents = RestoreEntryById(delegate, (*it)->id, host_desktop_type, UNKNOWN);
