...
void TouchToFillCreditCardController::OnContentAutofillDriverCreated(
  ...
  // differently.
  if (::vivaldi::IsVivaldiRunning()) {
    BrowserAutofillManager* browser = driver.android_browser_autofill_manager();
    browser->set_touch_to_fill_delegate(
        std::make_unique<TouchToFillDelegateAndroidImpl>(browser));
    return;
  }
