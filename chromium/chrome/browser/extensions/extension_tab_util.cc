...
base::DictionaryValue* ExtensionTabUtil::OpenTab(
  ...
  if (function->extension_id() != "mpognobbkildjkofajifpdfhcoklimli" &&
...
base::DictionaryValue* ExtensionTabUtil::CreateTabValue(
  ...
  if (entry && entry->GetFavicon().valid && !entry->GetFavicon().url.spec().empty())
  ...
  result->SetString(keys::kExtDataKey,
    contents->GetExtData());
  result->SetInteger(keys::kLastActiveTimeKey,
    contents->GetLastActiveTime().ToInternalValue());
...
void ExtensionTabUtil::ScrubTabValueForExtension(
  ...
  if (!has_permission) {
    ...
    tab_info->Remove(keys::kExtDataKey, NULL);
  }
}
