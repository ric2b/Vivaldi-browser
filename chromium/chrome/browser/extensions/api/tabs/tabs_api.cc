...
bool WindowsCreateFunction::RunSync() {
  ...
  for (std::vector<GURL>::iterator i = urls.begin(); i != urls.end(); ++i) {
    // TODO(gisli): If we want to be able to use url for the vivaldi tabs
    // we need to create Guest webcontents here.
...
bool TabsMoveFunction::MoveTab(int tab_id,
  ...
  // gisli@vivaldi.com:  Fix index bug.
  //*new_index += iteration;
  ...
  // gisli@vivaldi.com:  for within-window the new index depends on the
  // from pos.
  *new_index += tab_index >= *new_index ? 1 : 0;
  return true;
}
