...
#include "chrome/browser/extensions/api/editcommand/editcommand_api.h"
...
static WebContents* webContents() {
  ...
  AppWindow* appWindow = web_contents ?
      AppWindowRegistry::Get(web_contents->GetBrowserContext())->
          GetCurrentAppWindowForApp(vivaldi_app_id) : nullptr;
...
bool EditcommandExecuteFunction::RunAsync() {
  ...
  if (web_contents) {
    scoped_ptr<api::editcommand::Execute::Params> params(
      api::editcommand::Execute::Params::Create(*args_));
