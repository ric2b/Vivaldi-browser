...
ScopedJavaLocalRef<jobject> BookmarkBridge::CreateJavaBookmark(
  ...
  if (vivaldi::IsVivaldiRunning()) {
    ...
    return Java_BookmarkBridge_createVivaldiBookmarkItem(
        ...
        base::android::ConvertUTF8ToJavaString(env, vivaldi_bookmark_kit::GetNickname(node)),
        base::android::ConvertUTF8ToJavaString(env,
                                vivaldi_bookmark_kit::GetDescription(node)),
        vivaldi_bookmark_kit::GetThemeColor(node),
        java_timestamp,
        base::android::ConvertUTF8ToJavaString(env, vivaldi_bookmark_kit::GetThumbnail(node)),
        base::android::ConvertUTF8ToJavaString(env, node->uuid().AsLowercaseString())
        );
  }
