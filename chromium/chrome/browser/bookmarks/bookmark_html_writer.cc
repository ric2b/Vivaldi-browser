...
class Writer : public base::RefCountedThreadSafe<Writer> {
 public:
  Writer(const bookmarks::BookmarkModel* model,
    ...
    bookmarks_ =
        codec.Encode(model->bookmark_bar_node(), model->other_node(),
                              model->mobile_node(), model->trash_node(),
  ...
  void DoWrite() {
    ...
    base::Value::Dict* trash_folder_value =
        roots->FindDict(BookmarkCodec::kTrashBookmarkFolderNameKey);
    DCHECK(trash_folder_value);
    ...
    if (!WriteNode(*root_folder_value, BookmarkNode::BOOKMARK_BAR) ||
        ...
        (trash_folder_value &&
        !WriteNode(*trash_folder_value,BookmarkNode::TRASH))) {
