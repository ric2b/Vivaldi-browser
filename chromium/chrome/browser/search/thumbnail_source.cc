...
namespace {
GURL FixUrlIfNeeded(const std::string& path) {
  GURL url(path);

  // NOTE(pettern): The path has a tendency to be without
  // a valid protocol, so add one here if needed.
  if (!url.is_valid()) {
    std::string new_path = "http://" + path;
    GURL new_url(new_path);
    url.Swap(&new_url);
  }
  return url;
}
...
void ThumbnailSource::StartDataRequest(
  ...
  GURL url = FixUrlIfNeeded(page_url.spec());
  if (url.is_valid() &&
      thumbnail_service_->GetPageThumbnail(url, capture_thumbnails_,
                                           &data)) {
  ...
  if (url.is_valid() && capture_thumbnails_)
    thumbnail_service_->AddForcedURL(url);
}
