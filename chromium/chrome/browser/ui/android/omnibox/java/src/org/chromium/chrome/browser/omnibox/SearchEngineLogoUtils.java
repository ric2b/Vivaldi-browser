...
public class SearchEngineLogoUtils {
    ...
    public Promise<StatusIconResource> getSearchEngineLogo(
        ...
        //Vivaldi (gabriel@vivaldi.com) we have to distinguish search engine for private and regular
        String logoUrl;
        if (profile.isOffTheRecord()) {
            logoUrl = getPrivateSearchLogoUrl(templateUrlService);
        } else {
            logoUrl = getSearchLogoUrl(templateUrlService);
        }
        if (logoUrl == null) {
    ...
    private void processReturnedLogo(
        ...
        if (BuildConfig.IS_VIVALDI) {
            sCachedComposedImage = scaledIcon;
        } else {
    ...
    //Vivaldi
    public @Nullable String getPrivateSearchLogoUrl(@Nullable TemplateUrlService templateUrlService) {
        if (templateUrlService == null) return null;

        String logoUrlWithPath = templateUrlService.getUrlForSearchQuery(DUMMY_URL_QUERY, null,
