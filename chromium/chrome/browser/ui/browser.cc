...
Browser::CreateParams Browser::CreateParams::CreateForDevTools(
  ...
  // When we open an undocked devtools window in Vivaldi we do it via
  // WindowPrivateCreateFunction.
...
bool Browser::IsWebContentsVisible(
    content::WebContents* web_contents) {
...
void Browser::SetAsDelegate(WebContents* web_contents, bool set_delegate) {
  if (is_vivaldi()) {
    // Only take over as delegate if there is not already another delegate
    // This is particularly needed for discarded tabs, since they don't get
    // a webview.
    if (set_delegate && !web_contents->GetDelegate())
...
bool Browser::PopupBrowserSupportsWindowFeature(WindowFeature feature,
  ...
  switch (feature) {
    ...
    case FEATURE_TABSTRIP:
      ...
      // even in popups.
      if (is_vivaldi_ && app_name() != DevToolsWindow::kDevToolsApp)
        return true;
      U_FALLTHROUGH;
    case FEATURE_TOOLBAR:
    case FEATURE_BOOKMARKBAR:
