...
Browser::CreateParams::CreateParams(Profile* profile,
    ...
    : type(TYPE_TABBED),
      ...
      is_vivaldi(base::CommandLine::ForCurrentProcess()->IsRunningVivaldi() &&
...
Browser::Browser(const CreateParams& params)
  ...
  extensions::ExtensionActionUtil* extensionactionutils =
      extensions::ExtensionActionUtilFactory::GetForProfile(profile_);
...
Browser::~Browser() {
  ...
  tab_strip_model_->RemoveObserver(extensionactionutils);
#endif //ENABLE_EXTENSIONS
  ...
  // Vivaldi: We can't destroy the TabRestoreService if we have remaining
  // browsers. I.e. we only have a private window open with session data from
  // |profile_|.
  bool have_tabrestore_connections =
      BrowserList::GetInstance(chrome::GetActiveDesktop())->size() != 0 &&
      is_vivaldi();
  if (!chrome::GetTotalBrowserCountForProfile(profile_) &&
      !have_tabrestore_connections) {
...
void Browser::OnWindowClosing() {
  ...
  if (is_vivaldi() && tab_strip_model_->count() == 0) {
    // We have already called this and cleaned up.
    return;
  }
...
void Browser::TabMoved(WebContents* contents,
  ...
  // gisli@vivaldi.com:  This code should probably be in SyncHistoryWithTabs
  // but as some of the tab_strip_model events (for example TabDetached)
  // are fired BEFORE the active index is updated but others (like TabMoved)
  // is fired AFTER the active index is updated, this update code has to be
...
void Browser::ShowFirstRunBubble() {
  // gisli@vivaldi.com:  Added check for existance of the locationbar.
...
void Browser::ScheduleUIUpdate(WebContents* source,
  ...
  DCHECK(source);
  int index = tab_strip_model_->GetIndexOfWebContents(source);
  if(is_vivaldi_ && index == TabStripModel::kNoTab) {
    // |source| has already been removed from tabstrip, bail.
    return;
  }
  DCHECK_NE(TabStripModel::kNoTab, index);
  ...
  if (changed_flags & content::INVALIDATE_TYPE_URL && !is_vivaldi_) {
    if (source == tab_strip_model_->GetActiveWebContents()) {
  ...
  if (is_vivaldi_) {
    // If we Vivaldi we are not interest in scheduling any changes.
    return;
  }
