...
int TabStripModel::MoveWebContentsAt(int index,
  ...
  to_position = ConstrainMoveIndex(to_position, pinned, false);
...
int TabStripModel::MoveWebContentsAt(
  ...
  to_position = ConstrainMoveIndex(to_position, pinned, IsVivPanel(index));
  MoveTabToIndexImpl(index, to_position, group, pinned, select_after_move);
  return to_position;
}
...
void TabStripModel::MoveGroupTo(const tab_groups::TabGroupId& group,
  ...
  to_index = ConstrainMoveIndex(to_index, false /* pinned tab */, IsVivPanel(to_index));
...
void TabStripModel::AddTab(std::unique_ptr<tabs::TabModel> tab,
  ...
  // NOTE(andre@vivaldi.com): This will only happen for Vivaldi as we use the
  //  WebViewGuests inside the tabstrip. We cannot resize here as there is no
  //  WebContentsView in cross process frame guests.
...
int TabStripModel::ConstrainInsertionIndex(int index,
                                           bool pinned_tab,
                                           bool is_viv_panel) const {
  int res = pinned_tab ? std::clamp(index, 0, IndexOfFirstNonPinnedTab())
                    : std::clamp(index, IndexOfFirstNonPinnedTab(), count());
  return ConstrainVivaldiInsertionIndex(res, is_viv_panel);
}

int TabStripModel::ConstrainMoveIndex(int index,
                                      bool pinned_tab,
                                      bool is_viv_panel) const {
  int res = pinned_tab
             ? std::clamp(index, 0, IndexOfFirstNonPinnedTab() - 1)
             : std::clamp(index, IndexOfFirstNonPinnedTab(), count() - 1);
  return ConstrainVivaldiMoveIndex(res, is_viv_panel);
}
...
int TabStripModel::InsertTabAtImpl(
  ...
  index = ConstrainInsertionIndex(index, pin, tab->is_viv_panel());
...
bool TabStripModel::CloseWebContentses(
  ...
  if (!browser_shutdown::HasShutdownStarted()) {
    ...
    for (content::WebContents* contents : items) {
      ...
      // This prevents web widgets or panels renderers from being prematurely
      // terminated, avoiding the display of dead birds.
      if (vivaldi::ParentTabUserData::GetParentTabId(contents)) {
        auto visibility = contents->GetVisibility();
