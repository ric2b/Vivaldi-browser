...
void TabStripModel::SendDetachWebContentsNotifications(
  ...
  for (auto& dwc : notifications->detached_web_contents) {
    if (notifications->will_delete) {
      if (static_cast<content::WebContentsImpl*>(dwc->contents.get())
              ->RemoveChildFromOuter()) {
        // NOTE(andre@vivaldi.com) : If the WebContents is owned by a subframe
        // do not delete here.
        dwc->contents.release();
      } else {
...
bool TabStripModel::CloseWebContentses(
  ...
  for (size_t i = 0; i < items.size(); ++i) {
    ...
    if (!vivaldi::IsVivaldiRunning() || !vivaldi_force_tab_close_) {
      // clang-format off
...
const std::string TabStripModel::GetExtData(int index) const {
  WebContents* contents = GetWebContentsAtImpl(index);
  return contents ? contents->GetExtData() : std::string();
}
