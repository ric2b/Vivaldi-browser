...
void BrowsingHistoryHandler::HandleClearBrowsingData(
  ...
  if (vivaldi::IsVivaldiRunning()) {
    browser = vivaldi::FindVivaldiBrowser();
  } else {
    browser = chrome::FindBrowserWithTab(web_ui()->GetWebContents());
  }
...
void BrowsingHistoryHandler::OnQueryComplete(
  ...
  const syncer::DeviceInfoTracker* tracker =
      // Vivaldi: can be NULL when adding as panel in private mode (VB-88975)
      DeviceInfoSyncServiceFactory::GetForProfile(profile) ?
      DeviceInfoSyncServiceFactory::GetForProfile(profile)
          ->GetDeviceInfoTracker() : nullptr;
