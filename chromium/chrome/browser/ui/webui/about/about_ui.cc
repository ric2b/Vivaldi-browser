...
std::string ChromeURLs(content::BrowserContext* browser_context) {
  std::string html;
  AppendHeader(&html, "Vivaldi URLs");
  AppendBody(&html);
...
#else
  {
    ...
    for (const content::WebUIConfigInfo& info : infos) {
      ...
      std::string scheme = "vivaldi"; //info.origin.scheme();
      std::string url =
    ...
    html +=
        "</ul><a id=\"internals\"><h2>List of vivaldi://internals "
        "pages</h2></a>\n<ul>\n";
    ...
    for (const std::string_view path : sorted_internals_paths) {
      html += base::StrCat({"<li><a href='vivaldi://internals/", path,
...
#endif  // BUILDFLAG(IS_CHROMEOS_ASH)
    for (base::cstring_view url : chrome::ChromeDebugURLs()) {
      html += base::StrCat({"<li>vivaldi", url.substr(6), "</li>\n"});
    }
  }
  html += "</ul>\n";
