...
VersionUpdater* VersionUpdater::Create(content::WebContents* web_contents) {
  ...
  // Vivaldi restores tabs before a window has been created. So let us use an
  // AppWindow if available, NULL otherwise.
  ...
  if (base::CommandLine::ForCurrentProcess()->IsRunningVivaldi()) {
    ...
    if (browser) {
      ...
      extensions::AppWindowRegistry::const_iterator iter = app_windows.begin();
      extensions::AppWindow* appwindow = NULL;
      if (iter != app_windows.end()) {
        appwindow = *iter;
      }
      return new VersionUpdaterWin(appwindow ?
    ...
    if (!web_contents->GetTopLevelNativeWindow()) {
      return new VersionUpdaterWin(NULL);
    }
  }
