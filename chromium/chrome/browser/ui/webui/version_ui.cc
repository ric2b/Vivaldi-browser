...
#define BUILD_VERSION_final 0

#define S(s) BUILD_VERSION_ ## s
#define BUILD_VERSION(s) S(s)

#define VIVALDI_BUILD_VERSION_BETA 1

#undef VIVALDI_RELEASE
#define VIVALDI_RELEASE beta
...
WebUIDataSource* CreateVersionUIDataSource() {
  ...
  html_source->AddString("version", version_info.VivaldiVersion());
  ...
  html_source->AddString("cl", version_info.VivaldiLastChange());
#if defined(OFFICIAL_BUILD) &&  \
    BUILD_VERSION(VIVALDI_RELEASE) == VIVALDI_BUILD_VERSION_BETA
  html_source->AddString("official", VIVALDI_BETA_VERSION);
#else
  ...
  html_source->AddString("productLicense",
      l10n_util::GetStringFUTF16(IDS_ABOUT_VERSION_LICENSE,
