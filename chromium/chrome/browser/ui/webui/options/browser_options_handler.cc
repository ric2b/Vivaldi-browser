...
BrowserOptionsHandler::GetSyncStateDictionary() {
  ...
  signout_prohibited = SigninManagerFactory::GetForProfile(profile) &&
  ...
  sync_status->SetBoolean("signinAllowed", signin && signin->IsSigninAllowed());
  ...
  bool status_has_error = true;
  if (signin != NULL) {
    status_has_error = sync_ui_util::GetStatusLabels(profile, service, *signin,
  ...
  sync_status->SetBoolean("signedIn", signin && signin->IsAuthenticated());
