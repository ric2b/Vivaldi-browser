...
std::string BrowserTabRestoreServiceDelegate::GetExtData() const {
...
WebContents* BrowserTabRestoreServiceDelegate::ReplaceRestoredTab(
  ...
  return chrome::ReplaceRestoredTab(browser_, navigations, selected_navigation,
                             ...
                             session_storage_namespace, user_agent_override, ext_data);
}
...
TabRestoreServiceDelegate* TabRestoreServiceDelegate::Create(
  ...
  if (app_name.empty()) {
    ...
    params.is_vivaldi = base::CommandLine::ForCurrentProcess()->IsRunningVivaldi();
