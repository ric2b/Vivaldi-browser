...
bool StartupBrowserCreator::ProcessCmdLineImpl(
  ...
  if (command_line.HasSwitch(apps::kLoadAndLaunchApp) && can_use_profile) {
    ...
    if (!vivaldi::IsVivaldiRunning()) // We load the app in ComponentLoader.
  ...
  vivaldi::LaunchVivaldi(command_line, cur_dir, profile_info);
...
#if BUILDFLAG(IS_WIN)
  vivaldi::DoCleanShutdownIfNeeded(command_line);
#endif // BUILDFLAG(IS_WIN)
