...
void ManagePasswordsBubbleCocoa::Show(bool user_action) {
  ...
  if (!browserWindow) {
    // gisli@vivalid.com: For Vivaldi.
    browserWindow = webContents_->GetTopLevelNativeWindow();
  }
...
void ManagePasswordsBubbleCocoa::Show(content::WebContents* webContents,
  ...
  if (vivaldi::IsVivaldiRunning()) {
    // Vivaldi change so that window is not null when bubble is shown
    window = webContents->GetTopLevelNativeWindow();
  } else {
    window = [webContents->GetNativeView() window];
  }
  if (!window) {
  ...
  // Vivaldi addition, allow bwc and icon to be null
  ManagePasswordsIcon* icon = NULL;
  if (bwc) {
    ManagePasswordsDecoration* mpd = [bwc locationBarBridge]->manage_passwords_decoration();
    if (mpd) {
      icon = mpd->icon();
    }
  }
