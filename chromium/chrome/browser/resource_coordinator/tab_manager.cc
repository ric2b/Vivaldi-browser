...
void TabManager::OnBrowserSetLastActive(Browser* browser) {
  ...
  // NOTE(andre@vivaldi.com) : In Vivaldi we cannot start loading contents
  // before we are sure the transform is done from a regular WebContentsView to
  // a WebContentsViewGuest when GuestViewCrossProcessFrames turned off.
  // Activation is done in WebViewGuest::DidAttachToEmbedder.
  if (vivaldi::IsVivaldiRunning() && contents &&
      !contents->GetUserData("VivaldiWebContentsViewIsGuest")) {
    return;
  }
