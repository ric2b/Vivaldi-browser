...
#ifndef VIVALDI_SPARKLE_DISABLED
@synthesize sparkle_updater_delegate = _sparkle_updater_delegate;
@synthesize sparkle_updater_controller = _sparkle_updater_controller;
#endif
...
- (BOOL)validateUserInterfaceItem:(id<NSValidatedUserInterfaceItem>)item {
  ...
  if (action == @selector(commandDispatch:) ||
    ...
    if (_menuState &&  // NULL in tests.
      ...
      if (vivaldi::IsVivaldiRunning()) {
        bool hasWindow = !![NSApp keyWindow] || vivaldi::HasActiveWindow();
        bool enabled;
        if (::vivaldi::GetIsEnabled(tag, hasWindow, &enabled)) {
          return enabled;
        }
      }
...
#ifndef VIVALDI_SPARKLE_DISABLED
  [_sparkle_updater_delegate installUpdateAndRestart];
#endif
}

- (AutoUpdateStatus)getUpdateStatus {
#ifndef VIVALDI_SPARKLE_DISABLED
  return [_sparkle_updater_delegate getUpdateStatus];
#else
  return AutoUpdateStatus::kNoUpdate;
#endif
}

- (std::string)getUpdateVersion {
#ifndef VIVALDI_SPARKLE_DISABLED
  return [_sparkle_updater_delegate getUpdateVersion];
#else
  return "";
#endif
}
...
#ifndef VIVALDI_SPARKLE_DISABLED
  return [_sparkle_updater_delegate getUpdateReleaseNotesUrl];
#else
  return "";
#endif
}

// Called from menu when user wants to manually check for updates
      ...
      if (vivaldi::IsVivaldiRunning()) {
        ...
        if (!parameter.empty()) {
          if (Browser* browser = ActivateBrowser(profile)) {
            ...
            extensions::MenubarAPI::HandleActionById(
              browser->profile(), browser->session_id().id(), tag, parameter);
          }
          break;
        }
      }
      if (_menuState->IsCommandEnabled(tag)) {
        if (Browser* browser = ActivateBrowser(profile)) {
...
- (void)setVivaldiMenuItemAction:(NSMenuItem*)item {
    ...
    _menuState->UpdateCommandEnabled([item tag], true);
    switch ([item tag]) {
      case IDC_HIDE_APP:
      ...
      case IDC_VIV_SHOW_ALL:
        [item setAction:@selector(unhideAllApplications:)];
        break;
      case IDC_EXIT:
      ...
      case IDC_VIV_MAC_ZOOM:
        [item setAction:@selector(performZoom:)];
        break;
...
- (NSMenu*)applicationDockMenu:(NSApplication*)sender {
  ...
  if (IncognitoModePrefs::GetAvailability(profile->GetPrefs()) !=
    ...
    if (vivaldi::IsVivaldiRunning()) {
      titleStr = l10n_util::GetNSStringWithFixup(IDS_VIV_NEW_PRIVATE_WINDOW);
      item.title = titleStr;
      item.tag = IDC_VIV_NEW_PRIVATE_WINDOW;
    } else
...
- (void)setLastProfile:(Profile*)profile {
  ...
  if (vivaldi::IsVivaldiRunning()) {
    _profilePrefRegistrar->Add(
        ...
        base::BindRepeating(&vivaldi::ClearBookmarkMenu));
  }
}
  ...
  if (vivaldi::IsVivaldiRunning()) {
    for (auto url = urls.begin(); url != urls.end(); ++url) {
      if (url->scheme() == "mailto" &&
          vivaldi::HandleProtocol(profile, *url)) {
        return;
      }
    }
  } // end vivaldi
