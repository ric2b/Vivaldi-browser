          ...
          'actions': [
            {
              ...
              'inputs': [
                '<(reorder_py_path)',
                '$(OutDir)\\initialexe\\vivaldi.exe',
              ],
              'outputs': [
                '<(PRODUCT_DIR)\\vivaldi.exe',
                '<(PRODUCT_DIR)\\vivaldi.exe.pdb',
              ],
              'action': [
                'python',
                '<(reorder_py_path)',
    ...
    },
    {
      ...
      'conditions': [
        ...
        ['OS=="mac"', {
          ...
          'actions': [
            {
              ...
              'action': [
                ...
                '-o', '<(output_path)',
                '-t', 'main',
                '-V', '<(vivaldi_version_path)',
                '-B', '<(vivaldi_global_build_number)',
                '<@(locales)',
              ],
          ...
          'postbuilds': [
            {
			  # Patch the load location of the Sparkle framework
              'postbuild_name': 'Patch Vivaldi Framework',
              'action': [
                'install_name_tool',
                '-change',
                '@rpath/Sparkle.framework/Versions/A/Sparkle',
                '@rpath/Sparkle.framework/Sparkle',
                '${BUILT_PRODUCTS_DIR}/<(mac_product_name) Framework.framework/Vivaldi Framework',
              ],
            },
            {
            ...
            },
            {
              ...
              'action': ['<(tweak_info_plist_path)',
                         ...
                         '--vivaldi-build', '<(vivaldi_global_build_number)'],
            },
            {
          ...
          'copies': [
            {
              ...
              'files': [
                ...
                '<(VIVALDI)/app/other_resources/<(VIVALDI_RELEASE_KIND)/win8/SecondaryTile.png',
