...
std::string VersionInfo::VivaldiVersion() const {
...
std::string VersionInfo::VivaldiLastChange() const {
...
std::string VersionInfo::CreateVersionString() const {
  ...
  current_version += l10n_util::GetStringUTF8(IDS_ABOUT_VERSION_OFFICIAL);
#else
  ...
  current_version += " ";
#if defined(VIVALDI_BUILD)
  current_version += VivaldiLastChange();
#else
