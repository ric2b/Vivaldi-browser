...
class DefaultSearchProviderTest(ChromeEnterpriseTestCase):
  ...
  def test_default_search_provider_bing(self):
    self.SetPolicy('win2019-dc', 'DefaultSearchProviderEnabled', 1, 'DWORD')
    self.SetPolicy('win2019-dc', 'DefaultSearchProviderName', 'Bing', 'String')
    self.SetPolicy('win2019-dc', 'DefaultSearchProviderSearchURL',
                   '"https://www.bing.com/search?q={searchTerms}"', 'String')
    self.RunCommand('client2019', 'gpupdate /force')

    output = self._get_search_url('client2019')
    self.assertIn('www.bing.com', output)

  @test
  def test_default_search_provider_yahoo(self):
    self.SetPolicy('win2019-dc', 'DefaultSearchProviderEnabled', 1, 'DWORD')
    self.SetPolicy('win2019-dc', 'DefaultSearchProviderName', 'Yahoo', 'String')
    self.SetPolicy('win2019-dc', 'DefaultSearchProviderSearchURL',
  ...
  def test_default_search_provider_disabled(self):
    self.SetPolicy('win2019-dc', 'DefaultSearchProviderEnabled', 0, 'DWORD')
    self.RunCommand('client2019', 'gpupdate /force')
