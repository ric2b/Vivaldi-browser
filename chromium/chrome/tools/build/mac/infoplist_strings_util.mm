...
NSString* ApplicationVersionString(const char* version_file_path, const char* vivaldi_version_file_path, const char* vivaldi_build_num) {
  NSError* error = nil;
  ...
  if ([scanner scanString:@"MAJOR=" intoString:nil] &&
      ...
      if (vivaldi_build_num) {
          path_string = [NSString stringWithUTF8String:vivaldi_version_file_path];
          version_file =
              [NSString stringWithContentsOfFile:path_string
                                encoding:NSUTF8StringEncoding
                                   error:&error];
          ...
          if ([scanner scanString:@"VIVALDI_MAJOR=" intoString:nil] &&
              ...
              [scanner scanInt:&minor] &&
              [scanner scanString:@"VIVALDI_NIGHTLY=" intoString:nil] &&
              [scanner scanInt:&build] ) {
...
int main(int argc, char* const argv[]) {
  ...
  const char* vivaldi_build_num = NULL;
  const char* vivaldi_version_file_path = NULL;


  // Process the args
  int ch;
  while ((ch = getopt(argc, argv, "t:v:g:b:o:V:B:")) != -1) {
    switch (ch) {
      case 't':
        app_type = optarg;
        break;
