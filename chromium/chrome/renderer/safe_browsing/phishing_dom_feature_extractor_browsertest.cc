...
#if defined(OS_WIN)
#define MAYBE_SubFrames DISABLED_SubFrames
#else
#define MAYBE_SubFrames SubFrames
#endif
IN_PROC_BROWSER_TEST_F(PhishingDOMFeatureExtractorTest, MAYBE_SubFrames) {
  // This test doesn't exercise the extraction timing.
  EXPECT_CALL(clock_, Now()).WillRepeatedly(Return(base::TimeTicks::Now()));
...
//tomas@vivaldi.com: disabled browser test for win (VB-7468)
#if defined(OS_WIN)
#define MAYBE_SubframeRemoval DISABLED_SubframeRemoval
#else
#define MAYBE_SubframeRemoval SubframeRemoval
#endif
IN_PROC_BROWSER_TEST_F(PhishingDOMFeatureExtractorTest, MAYBE_SubframeRemoval) {
