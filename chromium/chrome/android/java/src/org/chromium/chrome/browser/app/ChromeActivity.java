...
public abstract class ChromeActivity<C extends ChromeActivityComponent>
    ...
    private static final float SPEED_DIAL_ASPECT_RATIO = 0.8125f;
    private AutomaticCloseTabsHandler mAutomaticCloseTabsHandler;
    private SyncReminderHandler mSyncReminderHandler;
    ...
    protected void onPreCreate() {
        ...
        if (BuildConfig.IS_OEM_MERCEDES_BUILD)
            Log.i("VIV_Activity_Lifecycle", "onPreCreate() called");
    ...
    public void performPostInflationStartup() {
        ...
        if (!BuildConfig.IS_FINAL_BUILD) {
            ...
            // By default, the watchdog will ignore ANRs if the debugger is attached or if the app
            // is waiting for the debugger to attach. This is because it detects execution pauses
                            ...
                            model.finishLoadingBookmarkModel(() -> {
                                if (model.getUserBookmarkIdForTab(tab) != null) {
                                    BookmarkItem item = model.getBookmarkById(
                                            new BookmarkId(model.getUserBookmarkIdForTab(tab).getId(), 0));
                                    if (SpeedDialUtils.thumbnailsEnabled() && item != null
                                            && model.isInsideSpeedDialFolder(item.getId())) {
                                        captureThumbnailForSpeedDial(model, item);
                                    }
                                }
                            });
                        }
    ...
    public void onStartWithNative() {
        ...
        if (BuildConfig.IS_OEM_GAS_BUILD)
            VivaldiUtils.initSensitiveDataPrefsState();
    }
    ...
    public void onStart() {
        // Vivaldi
        if (BuildConfig.IS_OEM_MERCEDES_BUILD)
            Log.i("VIV_Activity_Lifecycle", "onStart() called");
    ...
    public void onResume() {
        // Vivaldi
        if (BuildConfig.IS_OEM_MERCEDES_BUILD)
            Log.i("VIV_Activity_Lifecycle", "onResume() called");
    ...
    public void onStop() {
        // Vivaldi
        if (BuildConfig.IS_OEM_MERCEDES_BUILD)
            Log.i("VIV_Activity_Lifecycle", "onStop() called");
        super.onStop();
    ...
    protected final void onDestroy() {
        // Vivaldi
        if (BuildConfig.IS_OEM_MERCEDES_BUILD)
            Log.i("VIV_Activity_Lifecycle", "onDestroy() called");
        // Vivaldi - reset default search engine.
        if (ChromeApplicationImpl.isVivaldi()) {
            SearchEngineIconHandler.get().restoreDSE();
            if (mRootUiCoordinator != null &&
                    mRootUiCoordinator.getBottomSheetController() != null &&
                    mRootUiCoordinator.getBottomSheetController().isSheetOpen())
                mRootUiCoordinator.getBottomSheetController().collapseSheet(false);
        }
        ...
        // Vivaldi
        if (mAutomaticCloseTabsHandler != null) {
            mAutomaticCloseTabsHandler.destroy();
            mAutomaticCloseTabsHandler = null;
        }

        // Vivaldi
        if (mSyncReminderHandler != null) {
            mSyncReminderHandler = null;
        }
    ...
    public void finishNativeInitialization() {
        ...
        // (david@vivaldi.com): Close all tabs which weren't touched for a certain period of time.
        mAutomaticCloseTabsHandler = new AutomaticCloseTabsHandler(this,
    ...
    public boolean onMenuOrKeyboardAction(int id, boolean fromMenu) {
        ...
        if (id == R.id.help_id) {
            ...
            if (BuildConfig.IS_VIVALDI) {
                final int tabIndex = TabModelUtils.getTabIndexByUrl(getCurrentTabModel(),
                        VivaldiUtils.VIVALDI_HELP_AND_SUPPORT_URL);
                if (tabIndex != TabModel.INVALID_TAB_INDEX) {
                    TabModelUtils.setIndex(getCurrentTabModel(), tabIndex);
                } else {
                    boolean isIncognito = currentTab != null && currentTab.isIncognito();
                    getTabCreator(isIncognito).launchUrl(
                            VivaldiUtils.VIVALDI_HELP_AND_SUPPORT_URL,
                            TabLaunchType.FROM_CHROME_UI);
                }
            } else {
        ...
        if (id == R.id.home_menu_id) {
            VivaldiUtils.openHomepage(currentTab);
            return true;
        }
        ...
        if (id == R.id.auto_dark_web_contents_id || id == R.id.auto_dark_web_contents_check_id) {
            ...
            // Vivaldi: We don't show Chrome specific dialogs or record feature engagement
            if (!ChromeApplicationImpl.isVivaldi())
        ...
        if (id == R.id.vivaldi_reload_menu_id) {
            if (currentTab.isLoading())
                currentTab.stopLoading();
            else
                currentTab.reload();
        ...
        if (id == R.id.vivaldi_offline_page_id) {
            DownloadUtils.downloadOfflinePage(this, currentTab);
            return true;
        }
        if (id == R.id.launch_game_menu_id) {
            VivaldiUtils.loadVivaldiGame(currentTab);
            return true;
        }
    ...
    // Vivaldi: change of access modifier to access in ChromeTabbedActivity
    protected boolean doUniversalInstall(Tab currentTab) {
        ...
        PwaUniversalInstallBottomSheetCoordinator pwaUniversalInstallBottomSheetCoordinator =
                new PwaUniversalInstallBottomSheetCoordinator(
                        ...
                        R.drawable.vivaldi_logo_on_circular_background); // Vivaldi
    ...
    public void captureThumbnailForSpeedDial(BookmarkModel model, BookmarkItem item) {
        ...
        int width = getActivityTab().getView().getWidth();
    ...
    /** Vivaldi **/
    public TabBookmarker getTabBookMarker() {
        return mTabBookmarkerSupplier.get();
    }

    /** Vivaldi **/
    private void enableSensitiveDataManagersIfSecureOnAutomotive() {
        assert BuildConfig.IS_OEM_GAS_BUILD;
        VivaldiUtils.enableSensitiveDataManaging(VivaldiUtils.isDeviceSecure());

    }

    public PanelManager getPanelManager() {
        return mRootUiCoordinator.getPanelManager();
    }

    @Override
    public void onPause() {
        if (BuildConfig.IS_OEM_MERCEDES_BUILD)
            Log.i("VIV_Activity_Lifecycle", "onPause() called");
        super.onPause();
    }
}
