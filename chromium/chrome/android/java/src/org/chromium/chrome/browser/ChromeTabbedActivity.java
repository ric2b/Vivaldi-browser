...
import org.vivaldi.browser.bookmarks.ReadingListBottomSheet;
...
public class ChromeTabbedActivity extends ChromeActivity<ChromeActivityComponent>
    ...
    private boolean mDoubleBackPressed;
    ...
    public ChromeTabbedActivity() {
        ...
        mReauthenticationHelper = new PasswordAccessReauthenticationHelper(
                ContextUtils.getApplicationContext(), getSupportFragmentManager());
    }
        ...
        try {
            ...
            mTabModelObserver = new TabModelSelectorTabModelObserver(mTabModelSelector) {
                ...
                private void closeIfNoTabsAndHomepageEnabled(boolean isPendingClosure) {
                    ...
                    // Note(david@vivaldi.com): When tab strips are on, or we have a tablet device,
                    // we don't do anything.
                    if (SharedPreferencesManager.getInstance().readBoolean(
                                VivaldiPreferences.SHOW_TAB_STRIP, true)
                            || isTablet()) {
                        if (!mLayoutStateProviderSupplier.get().isLayoutVisible(
    ...
    private ViewGroup createAndSetStartSurfaceForTablet() {
        ...
        // Note(david@vivaldi.com): We always attach the tab switcher to the compositor view.
    ...
    public void startNativeInitialization() {
        try (TraceEvent e = TraceEvent.scoped("ChromeTabbedActivity.startNativeInitialization")) {
            ...
            if (BuildConfig.IS_OEM_AUTOMOTIVE_BUILD) {
                VivaldiUtils.maybeSetScaling(ContextUtils.getApplicationContext());
                VivaldiUtils.maybeAddDesktopSiteList(getCurrentTabModel().getProfile());
                if (VivaldiUtils.isCar(ContextUtils.getApplicationContext())) {
    ...
    public void onResumeWithNative() {
        ...
        if (mFromResumption) {
            ...
            // Note(david@vivaldi.com): Handle the authentication result here.
            if (VivaldiPreferences.getSharedPreferencesManager().readBoolean(
                        VivaldiPreferences.SCREEN_LOCK, false))
                if (!mReauthenticationHelper.canReauthenticate()) exitApplication();
        ...
        // Vivaldi
        if (VivaldiUtils.isCar(ContextUtils.getApplicationContext()) &&
                !CarDataProvider.getInstance().isVehicleParked())
            moveTaskToBack(true);
    }
    ...
    public void initializeState() {
        ...
        try {
            ...
            // Vivaldi
            if (!isMainIntentFromLauncher && Intent.ACTION_SEND.equals(intent.getAction())) {
                processSendIntent(intent);
            }
    ...
    public void performPreInflationStartup() {
        ...
        // Note(david@vivaldi.com): Show the authentication screen here if possible.
        if (VivaldiPreferences.getSharedPreferencesManager().readBoolean(
                    VivaldiPreferences.SCREEN_LOCK, false)) {
            if (!mReauthenticationHelper.canReauthenticate()) {
                mReauthenticationHelper.showScreenLockToast(
                        PasswordAccessReauthenticationHelper.ReauthReason.UNLOCK_APPLICATION);
                return;
            }
    ...
    protected RootUiCoordinator createRootUiCoordinator() {
        return new TabbedRootUiCoordinator(this, this::onOmniboxFocusChanged,
                ...
                this::getSnackbarManager, getActivityType(), this::isInOverviewMode, // Vivaldi
                this::isWarmOnResume,
        ...
        // Vivaldi specific:
            ...
            if (currentTab != null) {
                Tab newTab = getTabCreator(getCurrentTabModel().isIncognito())
                                     .launchUrl(currentTab.getUrl().getSpec(),
                                             TabLaunchType.FROM_CHROME_UI);
                // Note(david@vivaldi.com): In order to clone the previous tab properly, we need to
                // exchange the webcontents here.
                if (newTab != null)
            ...
            Runnable modelLoadedRunnable = () -> {
                ReadingListBottomSheet readingListBottomSheet = new ReadingListBottomSheet(this);
    ...
    public boolean handleBackPressed() {
        ...
        // Vivaldi
        // Close the page actions sheet if it is showing.
        if (mPageActionsSheet != null && mPageActionsSheet.close(true)) return true;
        if (isTablet() && mPanelManager != null && mPanelManager.onBackPressed()) return true;

        // Vivaldi
        final Tab curTab = getActivityTab();
        if (curTab != null && curTab.isNativePage()) {
            NativePage page = curTab.getNativePage();
            if (page != null && page.getUrl().equals(UrlConstants.NTP_URL)
                    && page instanceof SpeedDialPage
                    && ((SpeedDialPage)page).getManager().canGoBack()) {
                ((SpeedDialPage) page).getManager().goBack();
                return true;
            }
        }
    ...
    private boolean minimizeAppAndCloseTabOnBackPress(@Nullable Tab currentTab) {
        ...
        if (minimizeApp) {
            // Vivaldi - Check if double tap back button to exit is enabled
            if ( VivaldiUtils.isDoubleTapToExitEnabled()) {
                if (!mDoubleBackPressed ) {
                    Context context = ContextUtils.getApplicationContext();
                    Toast.makeText(context,
                            context.getString(R.string.double_tap_back_button_to_exit_toast),
                            Toast.LENGTH_SHORT)
                            .show();
                    mDoubleBackPressed = true;

                    // Reset the value of double tap after 2 seconds
                    new Handler().postDelayed(() -> mDoubleBackPressed = false, 2000);
                    return true;
                } else maybeClearSessionDataAndExit(); // Check and exit the application
            }
    ...
    public void onDestroyInternal() {
        ...
        if (mPanelManager != null) mPanelManager.onDestroyed();
    ...
    /** Vivaldi
     *  Processes share intent from external apps.
     */
    private void processSendIntent(Intent intent) {
        if ("text/plain".equals(intent.getType())) {
            String url = intent.getStringExtra(Intent.EXTRA_TEXT);
            if (url != null && !url.isEmpty()) {
                // If not a valid URL, use search.
                if (!URLUtil.isValidUrl(url)) {
                    url = TemplateUrlServiceFactory
                                  .getForProfile(Profile.getLastUsedRegularProfile())
                                  .getUrlForSearchQuery(url);
                    if (url == null || url.isEmpty()) return;
                }
                getTabCreator(false).launchUrl(url, TabLaunchType.FROM_EXTERNAL_APP);
            }
        }
    }
