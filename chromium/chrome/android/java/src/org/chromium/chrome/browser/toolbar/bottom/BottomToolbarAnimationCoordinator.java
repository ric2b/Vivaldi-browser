...
public class BottomToolbarAnimationCoordinator extends EmptyOverviewModeObserver {
    ...
    private void startAnimation(boolean showBrowsingMode) {
        ...
        // Note(david@vivaldi.com): We need this in order to avoid flickering between the browsing
        // mode toolbar and the tab switcher toolbar while opening the tab switcher.
        if (ChromeApplication.isVivaldi()) {
            mBrowsingModeCoordinator.setVisible(showBrowsingMode);
            mTabSwitcherModeCoordinator.setVisible(!showBrowsingMode);
        } else
        if (showBrowsingMode) {
