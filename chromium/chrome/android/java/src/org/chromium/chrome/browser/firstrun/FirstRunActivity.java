...
public class FirstRunActivity extends FirstRunActivityBase implements FirstRunPageDelegate {
    ...
    protected View createContentView() {
        ...
        if (ChromeApplicationImpl.isVivaldi()) { // Vivaldi VAB-10188
            return mPager;
        } // End Vivaldi
    ...
    public void onStart() {
        ...
        if (BuildConfig.IS_OEM_POLESTAR_BUILD) {
            if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.P ||
                    VivaldiUtils.isInvalidFingerprint()) {
                abortFirstRunExperience();
                Toast.makeText(this, R.string.vivaldi_invalid_os_version, Toast.LENGTH_LONG).show();
            }
        }
        // Vivaldi
        if (VivaldiUtils.shouldForceDarkMode()) {
            VivaldiPreferences.getSharedPreferencesManager()
                    .writeInt(ChromePreferenceKeys.UI_THEME_SETTING, ThemeType.DARK);
        }
