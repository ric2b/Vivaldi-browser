...
import org.vivaldi.browser.common.FaviconHandler;
import org.vivaldi.browser.startpage_dialog.DirectMatchBridge;
...
public class BookmarkImageFetcher {
    ...
    public void destroy() {
        ...
        if (ChromeApplicationImpl.isVivaldi() && mDirectMatchBridge != null)
            mDirectMatchBridge.destroy();
    }
    ...
    public void fetchFaviconForBookmark(BookmarkItem item, Callback<Drawable> callback) {
        ...
        // Vivaldi: Check direct match items for icons
        ...
        if (ChromeApplicationImpl.isVivaldi()) {
            if (imagePath != null) {
                new FaviconHandler().requestIconFromPath(imagePath, faviconDisplaySize,
                        faviconCallback);
                return;
            }
        } // End Vivaldi
