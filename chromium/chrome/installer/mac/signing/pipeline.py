        ...
        if do_notarization:
            ...
            if disable_packaging:
                for dist in config.distributions:
                    with commands.WorkDirectory(orig_paths) as paths:
                        dist_config = dist.to_config(config)
                        dest_dir = os.path.join(paths.output, dist_config.packaging_basename)
                        app_path = os.path.join(notary_paths.work, dist_config.packaging_basename, dist_config.app_dir)
                        commands.make_dir(dest_dir)
                        commands.move_file(app_path,
                                 os.path.join(dest_dir, dist_config.app_dir))
                        if dist_config.run_spctl_assess:
                            commands.run_command(['codesign', '--verify', '--verbose=6', "--deep", "--strict", os.path.join(dest_dir, dist_config.app_dir)])
                            commands.run_command(['spctl', '--assess', '-vvvv', os.path.join(dest_dir, dist_config.app_dir)])
