...
#if defined(USE_SYSTEM_PROPRIETARY_CODECS)
void GpuChannel::OnNewMediaPipeline(
    int32_t route_id,
  ...
  scoped_refptr<GpuChannelMessageQueue> queue =
      LookupStream(gpu_video_accelerator_factories_route_id);
  if (!queue)
    queue = CreateStream(gpu_video_accelerator_factories_route_id,
                         GpuStreamPriority::HIGH);

  IPCMediaPipeline* ipc_media_pipeline =
      new IPCMediaPipeline(this, route_id, command_buffer);
  AddRoute(route_id, gpu_video_accelerator_factories_route_id,
           ipc_media_pipeline);
  media_pipelines_.AddWithID(ipc_media_pipeline, route_id);
}

void GpuChannel::OnDestroyMediaPipeline(int32_t route_id) {
