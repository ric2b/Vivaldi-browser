...
bool DownloadResourceHandler::OnResponseStarted(
  ...
  // Vivaldi addition
  if(save_info_.get() && !save_info_.get()->prompt_for_save_location) {
    // if prompt has been set from "Save image as..." keep it.
    save_info_.get()->prompt_for_save_location = ask_for_target;
  }
...
void DownloadResourceHandler::ResumeRequest() {
  ...
  controller()->Resume(false); // TODO: andre@vivaldi.com maybe we should store this.
}
