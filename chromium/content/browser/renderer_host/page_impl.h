...
class CONTENT_EXPORT PageImpl : public Page {
 ...
 public:
  ...
  int vivaldi_loaded_resources() const { return loaded_resources_; }
  int vivaldi_total_resources() const { return total_resources_; }
  void vivaldi_reset_load_counters() {
    loaded_bytes_ = 0.0;
    loaded_resources_ = 0;
    total_resources_ = 0;
  }
  void vivaldi_update_load_counters(int64_t loaded_bytes_delta,
                                    int loaded_resource_delta,
                                    int started_resource_delta) {
    loaded_bytes_ += loaded_bytes_delta;
    loaded_resources_ += loaded_resource_delta;
    total_resources_ += started_resource_delta;

    // Due to asynchronous nature of events the reset may not be synchronous
    // with the update, so make sure that the total is always at least as big as
    // the loaded.
    if (total_resources_ < loaded_resources_) {
      total_resources_ = loaded_resources_;
    }
  }
