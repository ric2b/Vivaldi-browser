...
void UpdateWindowsKeyCodeAndKeyIdentifier(blink::WebKeyboardEvent* event,
  ...
  const char* id = GetKeyIdentifier(windows_key_code); //catch special keys
  if (id) {
    ...
    // in keyboard events.
    wchar_t keyId = windows_key_code;
#if defined(OS_WIN)
    ui::KeyboardCode key_code = (ui::KeyboardCode)windows_key_code;
    if (key_code >= '0' && key_code <= '9')
      keyId = static_cast<wchar_t>(key_code);
    else
      keyId = LOWORD(::MapVirtualKeyW(key_code, MAPVK_VK_TO_CHAR));
#endif
    // Revert to old behavior for other cases than our own
    base::snprintf(event->keyIdentifier,
                   sizeof(event->keyIdentifier),
                   "U+%04X",
                   base::ToUpperASCII(static_cast<int>(keyId)));
  }
}
