...
void RenderWidgetHostViewChildFrame::Init() {
  ...
  // NOTE(igor@vivaldi.com) Delay initialization of text input for Vivaldi tabs
  // until they are attached to the outer web_contents, VB-50662
  if (!vivaldi::IsVivaldiRunning() || !VivaldiTabCheck::IsVivaldiTabFrame(this))
  GetTextInputManager();
}
...
void RenderWidgetHostViewChildFrame::GestureEventAck(
  ...
  // render/blink, but from ch69 we do not receive CONSUMED_SHOULD_BUBBLE
  // (just CONSUMED) if the page being swiped contains elements with a Fixed
  // or Sticky positioning and the layer structure is not composited. I suspect
  // the latter depends on hardware as I see that on external monitors but I
  // guess it can happen on the primary as well dpeneding omn OS version and HW.
  // Regular chrome is not affected with this change as it always activate mac
  // history navigation in RenderWidgetHostImpl::OnGestureEventAck() as that is
  // where non-guestview mode acks are handeled.
...
void RenderWidgetHostViewChildFrame::ProcessTouchpadZoomEventAckInRoot(
  ...
  if (vivaldi::IsVivaldiRunning()) {
    RenderWidgetHostViewBase::ForwardTouchpadZoomEventIfNecessary(event, ack_result);
    return;
  }
