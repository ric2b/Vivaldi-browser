...
void RenderWidgetHostViewChildFrame::Init() {
  ...
  // VB-50662 VB-52024
  bool vivaldi_skip_text_init = false;
...
void RenderWidgetHostViewChildFrame::ProcessTouchpadZoomEventAckInRoot(
  ...
  if (vivaldi::IsVivaldiRunning()) {
    RenderWidgetHostViewBase::ForwardTouchpadZoomEventIfNecessary(event,
                                                                  ack_result);
    return;
  }
#endif // !IS_ANDROID && !IS_IOS
...
RenderWidgetHostViewChildFrame::FilterInputEvent(
  ...
  if (blink::WebInputEvent::IsPinchGestureEventType(input_event.GetType())) {
    ...
    if (vivaldi::IsVivaldiRunning() &&
      ...
      if (RenderViewHostImpl::From(host()) && static_cast<WebContentsImpl*>(
            WebContents::FromRenderViewHost(RenderViewHostImpl::From(host())))
            ->IsGuest()) {
        return blink::mojom::InputEventResultState::kNotConsumed;
      }
      // Return the same value as is done in the if statement below here
