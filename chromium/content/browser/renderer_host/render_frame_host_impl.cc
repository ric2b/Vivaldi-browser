...
void RenderFrameHostImpl::DownloadURL(
  ...
  // the DownloadInfo and does the permission handling.
  int process_host_id = GetProcess()->GetID();
  ...
  if (vivaldi::IsVivaldiRunning()) {
    ...
    while (!guest && current) {
      ...
      if (parent && !parent->ParentOrOuterDelegateFrame())
        guest = current;
      current = parent;
    };
    if(guest) {
      process_host_id = guest->GetProcess()->GetID();
...
void RenderFrameHostImpl::DidChangeLoadProgress(double load_progress) {
  ...
  // Vivaldi addition:
  // Add the sum of all the nodes to get proper totals.
  double loaded_bytes = 0;
  int total_elements = 0;
  int loaded_elements = 0;

  for (FrameTreeNode* n : frame_tree_node_->frame_tree()->Nodes()) {
    loaded_bytes += n->loaded_bytes();
    loaded_elements += n->loaded_elements();
    total_elements += n->total_elements();
    if (total_elements < loaded_elements) {
      total_elements = loaded_elements;
    }
  }
  GetPage().set_loaded_bytes(loaded_bytes);
  GetPage().set_loaded_elements(loaded_elements);
  GetPage().set_total_elements(total_elements);
