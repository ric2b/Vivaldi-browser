...
void StoragePartitionImpl::QuotaManagedDataDeletionHelper::
  ...
  for (const auto& bucket : buckets) {
    ...
    if (vivaldi::IsVivaldiRunning() &&
        vivaldi::IsVivaldiApp(bucket.storage_key.origin().host())) {
      continue;
    }
...
void StoragePartitionImpl::VivaldiUpdateBlobUrlRegistryWithFallback(
    storage::BlobUrlRegistry* fallback) {
  blob_url_registry_ =
      std::make_unique<storage::BlobUrlRegistry>(fallback->AsWeakPtr());
}
