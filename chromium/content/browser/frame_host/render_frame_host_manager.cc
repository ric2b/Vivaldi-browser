...
RenderFrameProxyHost* RenderFrameHostManager::GetProxyToParent() {
  if (frame_tree_node_->IsMainFrame()) {
    if (vivaldi::IsVivaldiRunning()) {
      // NOTE(andre@vivaldi.com) : Pass the proxy of the root app document,
      // which is the embedder. This will be the case for OOP-<WebViews>. See
      // https://www.chromium.org/developers/design-documents/oop-iframes
      // The reason for this deviation is that the webviews in Vivaldi can
      // change process during navigations.
      // See Vivaldi bug #VB-33910
      FrameTreeNode* outer = GetOuterDelegateNode();
      if (outer) {
        return GetProxyToOuterDelegate();
       }
    }
    return nullptr;
  }
...
void RenderFrameHostManager::DidNavigateFrame(
  ...
  if (vivaldi::IsVivaldiRunning()) {
    // NOTE(andre@vivaldi.com) : This is Vivaldi specific where a webview
    // navigates and the FrameTreeNode is already in focus and therefore will not
    // propegate focus states down until a document focus change has been done.
    RenderWidgetHostView* view = GetRenderWidgetHostView();
    if (view) {
      // Must set focus to the navigated view since a frame navigation calls, on
      // the Blink-side, |LocalFrame::Detach| causing the
      // |blink::FocusController| to miss focus changes to the newly navigated
      // Frame. Only do this when if a previous focus call failed and is_focused
      // and HasFocus is out of sync. See comment in the declaration for
      // RenderWidgetHostImpl::is_focused()
      bool is_focused =
          static_cast<RenderWidgetHostImpl*>(view->GetRenderWidgetHost())
              ->is_focused();
      bool has_focus = view->HasFocus();
      if (is_focused != has_focus)
        view->GetRenderWidgetHost()->Focus();
    }
  }
...
RenderFrameHostManager::GetSiteInstanceForNavigation(
  ...
  // Except for when running Vivaldi where WebViewGuests are used for pages.
...
void RenderFrameHostManager::CommitPending() {
  ...
  RenderFrameProxyHost* proxy_to_parent = GetProxyToParent();
  if (proxy_to_parent && proxy_to_parent->cross_process_frame_connector()) {
  ...
  if (!delegate_->IsHidden() && new_rfh_has_view) {
    ...
    // TODO(andre@vivaldi.com) : Fix this in a proper way without a resize.
    if (vivaldi::IsVivaldiRunning()) {
      gfx::Rect view_rect = render_frame_host_->GetView()->GetViewBounds();
      view_rect.set_height(view_rect.height() - 1);
      render_frame_host_->GetView()->SetBounds(view_rect);
      view_rect.set_height(view_rect.height() + 1);
      render_frame_host_->GetView()->SetBounds(view_rect);
    }
  }
  ...
  if (!new_rfh_has_view) {
    // NOTE (andre@vivaldi.com): In Vivaldi certain paths leading here. We will
    // create the renderview here.
    if (vivaldi::IsVivaldiRunning()) {
      RenderFrameProxyHost* existing_proxy =
          GetRenderFrameProxyHost(render_frame_host_->GetSiteInstance());
      if (frame_tree_node_->IsMainFrame()) {
        InitRenderView(render_frame_host_->render_view_host(), existing_proxy);
        delegate_->CreateRenderWidgetHostViewForRenderManager(
