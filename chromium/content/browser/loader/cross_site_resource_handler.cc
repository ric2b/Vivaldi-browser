...
bool CrossSiteResourceHandler::OnResponseStarted(
  ...
  return OnNormalResponseStarted(response, defer, open_when_done);
}
...
void CrossSiteResourceHandler::ResumeResponse(bool open_when_done,
                                              bool ask_for_target) {
  ...
  if (has_started_response_) {
    ...
    bool defer = false;
    if (!next_handler_->OnResponseStarted(response_.get(),
                                          &defer,
                                          open_when_done,
                                          ask_for_target)) {
      controller()->Cancel();
