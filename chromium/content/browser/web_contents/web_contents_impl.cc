...
WebContentsImpl::WebContentsTreeNode::~WebContentsTreeNode() {
  // NOTE(yngve) Condition disabled because it breaks a lot of unit tests
...
void WebContentsImpl::WebContentsTreeNode::OnFrameTreeNodeDestroyed(
  ...
  // In vivaldi we need to check if we have this webcontents in a tabstrip as
  // well as here since we use that to do sessions.
  if (VivaldiTabCheck::IsOwnedByTabStripOrDevTools(current_web_contents_)) {
    VivaldiDetachExternallyOwned(node);
    return;
  }
...
std::unique_ptr<WebContentsImpl> WebContentsImpl::CreateWithOpener(
  ...
  if (params.guest_delegate) {
    ...
    outer_web_contents = static_cast<WebContentsImpl*>(
        params.guest_delegate->GetOwnerWebContents());
    // NOTE(andre@vivaldi.com) : We need to track the current
    // browser_plugin_guest to be able to change this dynamically between
    // webcontents.
    params.guest_delegate->delegate_to_browser_plugin_ =
      new_contents->GetBrowserPluginGuest();
  }
...
base::ScopedClosureRunner WebContentsImpl::IncrementCapturerCount(
  ...
  observers_.NotifyObservers(&WebContentsObserver::CaptureStarted);
...
#if !BUILDFLAG(IS_ANDROID)
  // NOTE(david@vivaldi.com): |OnAudioStateChanged()| can be called when
  // WebContentsImpl is being destroyed. In this case we need to
  // make sure that we don't handle the audio state change. See (VB-45842).
  if (vivaldi::IsVivaldiRunning() && is_being_destroyed_)
    return;
#endif  // !IS_ANDROID
...
void WebContentsImpl::AttachInnerWebContents(
  ...
  if (is_full_page) {
    ...
    // NOTE(andre@vivaldi.com) :If the attaching is done for a background
    // content do not update focus.
    bool embedder_and_embedded_is_visible =
        GetVisibility() != Visibility::VISIBLE &&
        inner_web_contents_impl->GetVisibility() != Visibility::VISIBLE;
    if (embedder_and_embedded_is_visible) {
    inner_web_contents_impl->SetAsFocusedWebContentsIfNecessary();
    }
  }

  // NOTE(andre@vivaldi.com) : Used for book-keeping mounting and unmounting
  // webviews.
  inner_web_contents_impl->AttachedToOuter();
...
std::unique_ptr<WebContents> WebContentsImpl::DetachFromOuterWebContents() {
  ...
  // NOTE(andre@vivaldi.com) : This is needed in code-paths used in Vivaldi
  // since we support reparenting of WebContentsImpl. On Mac the path would lead
  // directly to an attach via
...
void WebContentsImpl::RecursivelyRegisterRenderWidgetHostViews() {
  ...
  // NOTE(igor@vivaldi.com): This also happens when the page's rendering
  // process crashes on out-of-memory or stack oveflow.
  if(GetRenderWidgetHostViewsInWebContentsTree().empty()) {
...
void WebContentsImpl::RecursivelyUnregisterRenderWidgetHostViews() {
  ...
  auto* text_input_manager = GetTextInputManager();
  for (auto* view : GetRenderWidgetHostViewsInWebContentsTree()) {
    if (text_input_manager && text_input_manager->IsRegistered(view)) {
...
std::unique_ptr<WebContents> WebContentsImpl::Clone() {
  ...
  // NOTE(andre@vivaldi.com) : Make sure we create the right type of
  // |WebContentsView| as in the same as the cloned content.
  create_params.always_create_guest = view_->IsWebContentsViewChildFrame();
...
KeyboardEventProcessingResult WebContentsImpl::PreHandleKeyboardEvent(
  ...
  if (outermost_contents != this && IsFullscreen() &&
    ...
    if (vivaldi::IsVivaldiRunning()) {
      // NOTE(igor@vibvaldi.com): We must redirect to a Vivaldi Tab, not the
      // outermost contents, to call WebViewGuest::PreHandleKeyboardEvent().
      //
      // If Chromium code changes here, we must review that code in
      // WebViewGuest.
      WebContents* vivaldi_tab = VivaldiTabCheck::GetOuterVivaldiTab(this);
      if (vivaldi_tab) {
        if (vivaldi_tab == this)
          goto call_delegate;
        outermost_contents = static_cast<WebContentsImpl*>(vivaldi_tab);
      }
    }
...
void WebContentsImpl::RequestToLockMouse(
  ...
  if (widget_in_frame_tree && delegate_) {
    for (WebContentsImpl* current = this;
         // Vivaldi: Don't set lock for GUI (VB-68886)
         current && !vivaldi::IsVivaldiApp(current->GetURL().host());
         current = current->GetOuterWebContents()) {
...
void WebContentsImpl::LostMouseLock(RenderWidgetHostImpl* render_widget_host) {
  OPTIONAL_TRACE_EVENT1("content", "WebContentsImpl::LostMouseLock",
                        "render_widget_host", render_widget_host);
  // Vivaldi: Don't remove lock for GUI. It doesn't have one. (VB-68886)
  if (!vivaldi::IsVivaldiApp(GetURL().host()) || mouse_lock_widget_) {
...
FrameTree* WebContentsImpl::CreateNewWindow(
  ...
  if (params.opener_suppressed) {
    // NOTE(andre@vivaldi.com) : Added check for Vivaldi here because of opening
    // a new tab/window from a guestview would break Vivaldis assumption of
    // WebViewGuests are tabs. See
    // https://source.chromium.org/chromium/chromium/src/+/134690e638999168acd893447c0244eaf2c0dde3
    // for related changes.
    if (is_guest && !vivaldi::IsVivaldiRunning()) {
  ...
  if (!new_contents) {
    // NOTE(andre@vivaldi.com) : Do not create as guest if we are using the
    // webcontents in a views::WebView control.
    if (!is_guest ||
        params.disposition == WindowOpenDisposition::NEW_PICTURE_IN_PICTURE) {
#if !BUILDFLAG(IS_ANDROID)
      if (vivaldi::IsVivaldiRunning()) {
        create_params.context = nullptr;
#if BUILDFLAG(IS_MAC)
        // NOTE(andre@vivaldi.com) : This is necessary on MAC because the initial
        // non-guest-view is not tore down before attaching to the parent.
        // Was VB-44298.
        create_params.always_create_guest = true;
#endif
      } else
#endif  // !IS_ANDROID
      {
      ...
      // This is a special case for extension popups opening links in private
      // windows.
      if (vivaldi::IsVivaldiRunning() && create_params.browser_context !=
          GetOuterWebContents()->GetBrowserContext()) {
        WebContentsDelegate* delegate = GetOuterWebContents()->GetDelegate();
        if (delegate) {

          const GURL target_url(create_params.initial_popup_url);

          std::unique_ptr<content::WebContents> passed_new_contents =
              WebContents::Create(create_params);

          delegate->AddNewContents(
              nullptr, std::move(passed_new_contents), target_url,
              params.disposition,
              blink::mojom::WindowFeatures(), true, nullptr);
        }
        // We do everything in VivaldiUIWebContentsDelegate.
        return nullptr;
      } // vivaldi && profile != profile2

      new_contents = GetBrowserPluginGuest()->CreateNewGuestWindow(
          create_params,
          static_cast<int>(params.disposition));
    }
    // NOTE(andre@vivaldi.com) : Since we navigate in CreateNewGuestWindow we
    // cannot set sessionstorage again here as it is already set. VB-98842.
    if (!vivaldi::IsVivaldiRunning() &&
        (!is_guest ||
         params.disposition == WindowOpenDisposition::NEW_PICTURE_IN_PICTURE)) {
...
#if !BUILDFLAG(IS_ANDROID)
  // Vivaldi : CreateNewGuestWindow can return nullptr if owner is null. This
  //           happens when closing a tab with blocked popups.
  if(vivaldi::IsVivaldiRunning() && !new_contents)
    return nullptr;
#endif  // !IS_ANDROID
  ...
  if (!params.opener_suppressed) {
    // NOTE(andre@vivaldi.com) : We need to create the webcontentsview for PDF
    // logic to work, otherwise we get a blank view when opening a pdf through a
    // data-url. Note this will cause DHECK hits in RenderFrameHostImpl::Init().
    // VB-97476
    if (!is_guest || vivaldi::IsVivaldiRunning()) {
  ...
  // NOTE(andre@vivaldi.com) : We cannot let the regular delegate take over for pip contents.
  if (delegate_ && params.disposition != WindowOpenDisposition::NEW_PICTURE_IN_PICTURE) {
...
void WebContentsImpl::DragSourceEndedAt(float client_x,
  ...
  VivaldiEventHooks::HandleDragEnd(this, operation, screen_x, screen_y);
...
void WebContentsImpl::ViewSource(RenderFrameHostImpl* frame) {
  ...
  CreateParams create_params = CreateParams(GetBrowserContext());
#if !BUILDFLAG(IS_ANDROID)
  if (IsGuest() && vivaldi::IsVivaldiRunning()) {
    view_source_contents = GetBrowserPluginGuest()->CreateNewGuestWindow(
        create_params,
        static_cast<int>(WindowOpenDisposition::NEW_FOREGROUND_TAB));
  } else {
    view_source_contents = WebContents::Create(create_params);
  }
#endif  // !IS_ANDROID
...
void WebContentsImpl::ResetLoadProgressState() {
  ...
  // Vivaldi addition:
  GetPrimaryPage().vivaldi_reset_load_counters();
}
...
void WebContentsImpl::RunJavaScriptDialog(
  ...
  // NOTE(andre@vivaldi.com) : Some dialogs can be shown before we have fully
  // started. The WebContentsDelegate is then the browser. Skip dialogs at this
  // stage.
#if !BUILDFLAG(IS_ANDROID)
  if (vivaldi::IsVivaldiRunning()) {
    // If this webcontents is the browser ui-document show this normally, if not
    // check if we are attached. If not this is a session restored webcontents
    // before Vivaldi has registered the correct dialog_manager_
    GURL url = GetURL();
    if (!vivaldi::IsVivaldiApp(url.host()) && !GetOuterWebContents()) {
...
void WebContentsImpl::EnsureOpenerProxiesExist(
  ...
  if (source_web_contents) {
    ...
    if (this != source_web_contents && GetBrowserPluginGuest()) {
      // NOTE(andre@vivaldi.com) : No need for a RenderFrameProxy if both
      // contents has the same siteinstance. The RenderFrameProxy should only be
      // created for cross-site navigations, as
      // FrameTree::CreateProxiesForSiteInstance does. (Which is done for
      // non-guests below.)
      if (source_web_contents->GetSiteInstance() == GetSiteInstance()) {
        return;
      }
...
void WebContentsImpl::FocusOwningWebContents(
...
#if !BUILDFLAG(IS_ANDROID)
  // NOTE(daniel@vivaldi.com): If browser_plugin_guest_ is NULL then it must
  // be the Vivaldi UI. Don't focus.
  if (vivaldi::IsVivaldiRunning() && !browser_plugin_guest_) {
    return;
  }
#endif  // !IS_ANDROID
  ...
  // NOTE(andre@vivaldi.com) : Switching focus with the mouse from the Vivaldi
  // UI to an extension popup/iframe will fail because the delegate is the same
  // for both, this |WebContentsImpl|.
  if (vivaldi::IsVivaldiRunning() ||
     (focused_widget != render_widget_host &&
      (!focused_widget ||
...
void WebContentsImpl::CreateRenderWidgetHostViewForRenderManager(
  ...
  if (is_inner_frame_tree) {
    ...
    // NOTE(andre@vivaldi.com) :  this was because the size of the frame was not
    // set initially and could cause null size. Viewrect set in
    // EmbeddedContentView::SetFrameRect.
    view->SetSize(GetSizeForMainFrame());
...
void WebContentsImpl::SetBrowserPluginGuest(
  ...
  // Vivaldi, we must re-set the browser plugin when moving tabs between windows
  // or when a tab is restored.
#if !BUILDFLAG(IS_ANDROID)
  if (!vivaldi::IsVivaldiRunning()) {
...
void WebContentsImpl::DecrementCapturerCount(bool stay_hidden,
  ...
  observers_.NotifyObservers(&WebContentsObserver::CaptureFinished);
}
...
base::ScopedClosureRunner WebContentsImpl::CreateDisallowCustomCursorScope(
  ...
  // NOTE(andre@vivaldi.com) : In case of guest-views. VB-97431.
  if (!cursor_manager) {
      WebContentsImpl* outer_contents = GetOuterWebContents();
    if (!outer_contents) {
      return base::ScopedClosureRunner();
    }
    auto* root_view = outer_contents->GetPrimaryMainFrame()
                                      ->GetRenderWidgetHost()
                                      ->GetRenderWidgetHostViewBase();
    cursor_manager = root_view->GetCursorManager();
  }
