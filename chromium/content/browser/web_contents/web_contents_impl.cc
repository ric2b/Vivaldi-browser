...
base::OnceClosure WebContentsImpl::AddReceiverSet(
  ...
  if (vivaldi::IsVivaldiRunning()) {
  // We need to this as ChromeWebViewPermissionHelperDelegate is called once
  // per WebViewGuest::Init().
...
#if !defined(OS_ANDROID)
      if (vivaldi::IsVivaldiRunning()) {
        int initial_x = params.features->x;
        int initial_y = params.features->y;
        int initial_width = params.features->width;
        int initial_height = params.features->height;
        initial_rect =
            gfx::Rect(initial_x, initial_y, initial_width, initial_height);
      }
#endif  // !OS_ANDROID
...
void WebContentsImpl::DragSourceEndedAt(float client_x,
  ...
  // dropping tabs outside a window, we add this extra event. On Windows this
  // is called from the callee to get access to a platform-specific drag cancel
  // flag.
  if (::vivaldi::IsVivaldiRunning()) {
    if (VivaldiEventHooks* h = VivaldiEventHooks::FromWebContents(this)) {
      h->HandleDragEnd(operation, false, screen_x, screen_y);
    }
  }
...
#if !defined(OS_ANDROID)
  if (BrowserPluginGuest::IsGuest(this) && vivaldi::IsVivaldiRunning()) {
    view_source_contents = base::WrapUnique(
        GetBrowserPluginGuest()->CreateNewGuestWindow(create_params));
  } else {
...
void WebContentsImpl::SendChangeLoadProgress() {
  ...
  for (auto& observer : observers_) {
    ...
    observer.ExtendedLoadProgressChanged(
        frame_tree_.load_progress(), frame_tree_.loaded_bytes(),
        frame_tree_.loaded_elements(), frame_tree_.total_elements());
  }
}
...
void WebContentsImpl::FocusOwningWebContents(
  ...
  // NOTE(david@vivaldi.com): Make sure the outer web contents knows our frame
  // is focused.
  if (vivaldi::IsVivaldiRunning()) {
    WebContentsImpl* outermost = GetOutermostWebContents();
    if (this != outermost && !ContainsOrIsFocusedWebContents())
