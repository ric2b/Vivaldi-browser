...
FrameTree* WebContentsImpl::CreateNewWindow(
  ...
  if (!new_contents) {
    ...
    if (!is_guest ||
      ...
      // windows.
      // This can be the outermost WebContents so we need to check that. VB-101579.
      WebContentsImpl* outer_contents = GetOuterWebContents();
      if (vivaldi::IsVivaldiRunning() &&
          (outer_contents && create_params.browser_context !=
                                 outer_contents->GetBrowserContext())) {
  ...
  if (params.opener_suppressed) {
    ...
    if (!was_blocked) {
      ...
      // NOTE(andre@vivaldi.com) : This fixed
      // ShouldResumeRequestsForCreatedWindow for webviews. Navigation is done
      // on attachment. Fixes VB-94302 and VB-99904.
      if (delegate_ && (!is_guest || vivaldi::IsVivaldiRunning()) &&
