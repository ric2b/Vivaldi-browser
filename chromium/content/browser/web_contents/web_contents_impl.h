...
class CONTENT_EXPORT WebContentsImpl : public WebContents,
 ...
 public:
  ...
  // Vivaldi specific:
  void DetachFromOuter();

  void SetExtData(const std::string& ext_data) override;
  const std::string& GetExtData() const override;
  // If we are attached to another webcontents disconnect. Returns true if we
  // disconnected and deleted child. Used in TabStripModel.
  bool RemoveChildFromOuter();
 ...
 private:
  ...
  class WebContentsTreeNode final : public FrameTreeNode::Observer {
   public:
    ...
    // Vivaldi specific, this assumes that the outer webcontents
  ...
  // NOTE(andre@vivaldi.com) : Used for tracking when it is safe to create a
  // webviewguest after detaching.
  void FrameTreeNodeDestroyed();
  void AttachedToOuter();
