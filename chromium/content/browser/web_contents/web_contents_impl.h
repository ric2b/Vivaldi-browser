 ...
 private:
  ...
  class WebContentsTreeNode final : public FrameTreeNode::Observer {
   ...
   private:
    ...
    // Vivaldi specific to detach externally owned contents from its outer
    // contents when the corresponding FrameTreeNode from the outer contents is
    // destroyed.
    void VivaldiDetachExternallyOwned(FrameTreeNode* node);

    void VivaldiDestructor();
