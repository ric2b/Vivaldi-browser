...
bool WebPluginDelegateImpl::Initialize(
  ...
  int argsize = arg_names.size() + 1;

  scoped_ptr<char*[]> argn(new char*[argsize]);
  scoped_ptr<char*[]> argv(new char*[argsize]);
  ...
  if (base::CommandLine::ForCurrentProcess()->IsRunningVivaldi() ||
    ...
    if (instance_->plugin_lib()->plugin_info().name.find(
        base::ASCIIToUTF16("Shockwave Flash")) != std::wstring::npos) {
      //flash (<URL:http://helpx.adobe.com/flash/kb/flash-object-embed-tag-attributes.html>)
      argn[argc] = const_cast<char *>("wmode");
      argv[argc] = const_cast<char *>("opaque");
      argc++;
    }
    else if (instance_->plugin_lib()->plugin_info().name.find(
             base::ASCIIToUTF16("Silverlight")) != std::wstring::npos) {
      //silverlight (<URL:http://msdn.microsoft.com/en-us/library/bb979728(v=vs.95).aspx>)
      argn[argc] = const_cast<char *>("Windowless");
      argv[argc] = const_cast<char *>("True");
      argc++;
    }
  }
