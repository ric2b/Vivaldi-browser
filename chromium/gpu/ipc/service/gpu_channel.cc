...
class GPU_IPC_SERVICE_EXPORT GpuChannelMessageFilter
 ...
 private:
  ...
  void VivaldiStartNewMediaPipeline(
      gpu::mojom::VivaldiMediaPipelineParamsPtr params) override;
  void VivaldiDestroyMediaPipeline(int32_t route_id) override;

  // End of Vivaldi additions
...
void GpuChannelMessageFilter::VivaldiStartNewMediaPipeline(
  ...
  main_task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&PropmediaGpuChannel::StartNewMediaPipeline,
                                gpu_channel_->AsWeakPtr(), std::move(params)));
#endif
}

void GpuChannelMessageFilter::VivaldiDestroyMediaPipeline(int32_t route_id) {
  base::AutoLock lock(gpu_channel_lock_);
  if (!gpu_channel_) {
    receiver_.reset();
    return;
...
#if defined(USE_SYSTEM_PROPRIETARY_CODECS)
  main_task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&PropmediaGpuChannel::DestroyMediaPipeline,
                                gpu_channel_->AsWeakPtr(), route_id));
#endif
}
