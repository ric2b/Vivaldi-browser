...
void MessagingAPIMessageFilter::OnOpenChannelToExtension(
  ...
  if (info.source_url == kUnreachableWebDataURL) {
    // NOTE(andre@vivaldi.com) : We can get info.source_url ==
    // kUnreachableWebDataURL when a frame is collapsed, by ad-filtering or when
    // a frame has a load-error. See NavigationRequest::DidCommitNavigation:
    //  The last committed load in collapsed frames will be an error page with
    //  |kUnreachableWebDataURL|.
    // VB-94508, and
    // https://source.chromium.org/chromium/chromium/src/+/288f8309f6e624b8a327869c9d68d3dfaa588bf4
    return;
  }
