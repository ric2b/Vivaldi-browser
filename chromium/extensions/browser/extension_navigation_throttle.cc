...
ExtensionNavigationThrottle::WillStartOrRedirectRequest() {
  ...
  if (url_has_extension_scheme) {
    ...
    // If this is a <webview> owned by Vivaldi let the navigation through. This
    // was introduced for VB-85798 where webstore navigation inside webviews was
    // blocked. This must be restricted to Vivaldi owned webviews. Note that
    // other extensions do not have permission to access webviews, so this is
    // safe.
