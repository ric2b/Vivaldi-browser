...
def reorder_imports(input_dir, output_dir, architecture):
  """Run swapimports.exe on the initial vivaldi.exe, and write to the output
  directory. Also copy over any related files that might be needed
  (pdbs, manifests etc.).
  """

  input_image = os.path.join(input_dir, 'vivaldi.exe')
  output_image = os.path.join(output_dir, 'vivaldi.exe')
  ...
  args.append('vivaldi_elf.dll');

  subprocess.call(args)

  for fname in glob.iglob(os.path.join(input_dir, 'vivaldi.exe.*')):
...
def main(argv):
  ...
  parser.add_option('-i', '--input', help='reorder vivaldi.exe in DIR',
      metavar='DIR')
  parser.add_option('-o', '--output', help='write new vivaldi.exe to DIR',
      metavar='DIR')
