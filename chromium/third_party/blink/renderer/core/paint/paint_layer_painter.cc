...
PaintResult PaintLayerPainter::PaintSingleFragment(
  ...
  if (&paint_layer_ != painting_info.root_layer) {
    ...
    // NOTE(igor@vivaldi): This flag should be applied only to the top paint
    // layer when we capture the full page. The background for nested layers
    // should be always properly clipped.
    paint_flags &= ~kPaintLayerPaintingWholePageBackground;
  }
