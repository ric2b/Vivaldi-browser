...
NEW_STAMP_FILE = os.path.normpath(
    os.path.join(LLVM_BUILD_DIR, 'cr_build_revision'))
...
def UpdateClang():
  ...
  if ReadStampFile(STAMP_FILE) == expected_stamp:
    if not os.access(NEW_STAMP_FILE, os.F_OK):
      return 0
    if ReadStampFile(NEW_STAMP_FILE) == expected_stamp:
      return 0
  ...
  WriteStampFile(expected_stamp, NEW_STAMP_FILE)
