...
def Run(project_config, no_browser=False, disable_name=None):
  args = sys.argv[1:]
  ...
  # Vivaldi code for centrally disabling telemetry tests
  if disable_name:
    try:
      sys.path.append(os.path.join(telemetry_dir,
                    "..", "..", "..", "testing", disable_name))
      disable_telemetry = __import__('disable_'+  disable_name)

      args.extend(disable_telemetry.get_disabled_tests("--skip"));
    except:
