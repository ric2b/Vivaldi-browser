...
class _OsxPlatformSettings(_PosixPlatformSettings):
  ...
  def _get_dns_service_key(self):
    ...
    for line in lines:
      ...
      if key_value[0].strip() == 'PrimaryService':
        return [
            'Setup:/Network/Service/%s/DNS' % key_value[1].strip(),
            'State:/Network/Service/%s/DNS' % key_value[1].strip(),
            ]
    raise DnsReadError('Unable to find DNS service key: %s'%(output))

  def _get_primary_nameserver(self):
    keys = self._get_dns_service_key()
    output_list = []
    for key in keys:
      output = self._scutil('show %s' % key)
    ...
    raise DnsReadError('Unable to find primary DNS server: %s : %s' %(str(keys), str(output_list)))
