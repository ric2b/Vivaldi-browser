...
base::Value BookmarkCodec::Encode(
  ...
  if (trash_folder_node)
    roots.SetKey(kTrashBookmarkFolderNameKey, EncodeNode(trash_folder_node));
...
bool BookmarkCodec::DecodeHelper(BookmarkNode* bb_node,
  ...
  // didn't have it.
  if (trash_folder_node) {
    const base::Value* trash_folder_value = roots->FindDictKey(kTrashBookmarkFolderNameKey);
