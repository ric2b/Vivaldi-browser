...
bool BookmarkModel::IsLocalOnlyNode(const BookmarkNode& node) const {
  ...
  return ancestor_permanent_node != account_bookmark_bar_node_ &&
         ...
         ancestor_permanent_node != account_trash_node_;
}
    ...
    if (key == vivaldi_bookmark_kit::ThumbnailString()) {
      ...
      if (checksum) {
        vivaldi_synced_file_store_->SetLocalFileRef(
            node->uuid(), syncer::BOOKMARKS, *checksum);
      } else {
        vivaldi_synced_file_store_->RemoveLocalRef(node->uuid(),
                                                   syncer::BOOKMARKS);
      }
    }
  }
...
const BookmarkNode* BookmarkModel::AddURL(
  ...
  if (meta_info && vivaldi_synced_file_store_) {
    ...
    if (checksum) {
      vivaldi_synced_file_store_->SetLocalFileRef(new_node->uuid(),
...
void BookmarkModel::DoneLoading(std::unique_ptr<BookmarkLoadDetails> details) {
  ...
  account_trash_node_ = details->account_trash_folder_node();
...
void BookmarkModel::RemoveNodeFromIndicesRecursive(
  ...
  if (vivaldi_synced_file_store_)
    vivaldi_synced_file_store_->RemoveLocalRef(node->uuid(), syncer::BOOKMARKS);
...
void BookmarkModel::CreateAccountPermanentFolders() {
  ...
  if (vivaldi::IsVivaldiRunning()) {
    std::unique_ptr<BookmarkPermanentNode> account_trash_node =
        BookmarkPermanentNode::CreateTrashFolder(next_node_id_++);
    account_trash_node_ = account_trash_node.get();
    AddNode(root_, root_->children().size(), std::move(account_trash_node),
            /*added_by_user=*/false, NodeTypeForUuidLookup::kAccountNodes);
  }
}
...
BookmarkStorage* BookmarkModel::GetStorageForNode(const BookmarkNode* node) {
  ...
  if (permanent_node == account_bookmark_bar_node_ ||
      ...
      permanent_node == account_trash_node_) {
    CHECK(AreFoldersForAccountStorageAllowed());
...
// Vivaldi
const BookmarkNode* BookmarkModel::account_trash_node() const {
  DCHECK_CALLED_ON_VALID_SEQUENCE(sequence_checker_);
  // Must be null if the feature flag isn't enabled.
  CHECK(!account_trash_node_ || AreFoldersForAccountStorageAllowed());
  return account_trash_node_;
}
}  // namespace bookmarks
