...
void AutocompleteController::InitializeSyncProviders(int provider_types) {
  ...
  if (vivaldi::IsVivaldiRunning()) {
    if (provider_types & AutocompleteProvider::TYPE_BOOKMARK_NICKNAME) {
      bookmark_nickname_provider_ =
          new BookmarkNicknameProvider(provider_client_.get());
      providers_.push_back(bookmark_nickname_provider_.get());
    }
    if (provider_types & AutocompleteProvider::TYPE_DIRECT_MATCH) {
      direct_match_provider_ = new DirectMatchProvider(provider_client_.get());
      providers_.push_back(direct_match_provider_.get());
    }
  }
