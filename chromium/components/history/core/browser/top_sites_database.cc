...
bool TopSitesDatabase::InitImpl(const base::FilePath& db_name) {
  ...
  // Vivaldi upgrade code to move thumbnails into our own storage.
  if (vivaldi::IsVivaldiRunning() && meta_table_.GetVersionNumber() == 3 &&
      !convert_callback_.is_null()) {
    ConvertThumbnailData(db_name, std::move(convert_callback_));
  }
