...
bool FakeServer::CreatePermanentNotesFolder(const std::string& server_tag,
  const std::string& name) {
  DCHECK(thread_checker_.CalledOnValidThread());
  std::unique_ptr<FakeServerEntity> entity =
    PermanentEntity::Create(syncer::NOTES, server_tag, name,
...
string FakeServer::CommitEntity(
  ...
  } else  if (GetModelType(client_entity) == syncer::NOTES) {
    EntityMap::const_iterator iter = entities_.find(client_entity.id_string());
    if (iter != entities_.end()) {
      entity = NotesEntity::CreateUpdatedVersion(
        client_entity,
        *iter->second,
        parent_id);
    } else {
      entity = NotesEntity::CreateNew(client_entity, parent_id, client_guid);
    }
  } else {
