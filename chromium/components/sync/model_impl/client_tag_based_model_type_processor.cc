...
bool ClientTagBasedModelTypeProcessor::HasLocalChanges() const {
  for (const auto& kv : entities_) {
    ...
    if (entity->RequiresCommitRequest() && !entity->FailedGettingCommitData()) {
      return true;
    }
  }
  return false;
...
void ClientTagBasedModelTypeProcessor::GetLocalChanges(
  ...
  for (const auto& kv : entities_) {
    ...
    if (entity->RequiresCommitData()) {
      ...
      // NOTE(julien): This is a workaround for VB-43098. We note here that we
      // tried to load commit data, and if they do not get loaded, we don't
      // let those entities trigger nudges on subsequent update cycles ( in
      // HasLocalChanges ), since we expect that they won't get more successful
      // at obtaining these commit data. See also
      // https://bugs.chromium.org/p/chromium/issues/detail?id=871733 which
      // tracks chromium's effort at fixing this properly.
      entity->SetCommitDataRequested();
    }
  }
