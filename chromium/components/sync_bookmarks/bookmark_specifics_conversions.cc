...
void UpdateBookmarkNodeFromSpecifics(
  ...
  if (synced_thumbnail == kVivaldiAutogeneratedMarker) {
    if (vivaldi_data_url_utils::GetSyncedStoreChecksumForUrl(thumbnail) ||
        vivaldi_data_url_utils::IsResourceURL(thumbnail) || thumbnail.empty()) {
  ...
  // missing ones.
  if (synced_thumbnail.empty() && !thumbnail.empty())
      map[vivaldi_bookmark_kit::ThumbnailString()] = thumbnail;
  ...
  // NOTE(julien@vivaldi.com): non-migrated custom thumbnails should not be
  // replaced. We also want them to take over when they do get migrated.
  if (vivaldi_data_url_utils::IsLocalPathUrl(thumbnail))
      map[vivaldi_bookmark_kit::ThumbnailString()] = thumbnail;
