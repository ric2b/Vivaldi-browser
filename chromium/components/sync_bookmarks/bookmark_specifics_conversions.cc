...
const char kVivaldiAutogeneratedMarker[] = "AUTOGENERATED";
...
void UpdateBookmarkSpecificsMetaInfo(
  ...
  for (const auto& [key, value] : *metainfo_map) {
    if (key == vivaldi_bookmark_kit::ThumbnailString() &&
        vivaldi_data_url_utils::IsLocalPathUrl(value)) {
      // Ignore old custom bookmark thumbnail URLs. These will eventually be
      // moved to the synced file store and get a new, committable URL.
      continue;
    }
    ...
    if (key == vivaldi_bookmark_kit::ThumbnailString() &&
        !vivaldi_data_url_utils::GetSyncedStoreChecksumForUrl(value) &&
        !vivaldi_data_url_utils::IsResourceURL(value, nullptr)) {
      // Autogenerated URLs are not usable on other clients. Replace them with a
      // recognizable string.
      meta_info->set_value(kVivaldiAutogeneratedMarker);
    }
  }
}
...
const bookmarks::BookmarkNode* CreateBookmarkNodeFromSpecifics(
  ...
  if (synced_thumbnail == kVivaldiAutogeneratedMarker) {
    metainfo[vivaldi_bookmark_kit::ThumbnailString()] = "";
  }
...
void UpdateBookmarkNodeFromSpecifics(
  ...
  std::string remote_thumbnail = vivaldi_bookmark_kit::GetThumbnail(map);
  std::string local_thumbnail = vivaldi_bookmark_kit::GetThumbnail(node);

  DLOG(INFO) << "UpdateBookmarkNodeFromSpecifics \n remote_thumbnail : "
             << remote_thumbnail << "\n local_thumbnail : " << local_thumbnail;

  // NOTE(julien@vivaldi.com): Missing autogenerated thumbnails are represented
  // using a present, but empty thumbnail URL on the client-side. This should in
  // most case caused the client to behave as if the thumbnail is missing, but
  // UpdateBookmarkSpecificsMetaInfo will still recognize it as autogenerated.
  // If an autogenerated thumbnail already exists, it is not replaced.
  if (remote_thumbnail == kVivaldiAutogeneratedMarker) {
    if ((vivaldi_data_url_utils::GetSyncedStoreChecksumForUrl(
             local_thumbnail) ||
         vivaldi_data_url_utils::IsResourceURL(local_thumbnail) ||
         local_thumbnail.empty())) {
      map[vivaldi_bookmark_kit::ThumbnailString()] = "";
    } else {
      map[vivaldi_bookmark_kit::ThumbnailString()] = local_thumbnail;
    }
  }
  // NOTE(julien@vivaldi.com): existing thumbnails always take precedence over
  // missing ones.
  bool has_local_thumbnail_no_remote =
      remote_thumbnail.empty() && !local_thumbnail.empty();
  bool local_thumbnail_is_non_migrated_custom =
      vivaldi_data_url_utils::IsLocalPathUrl(local_thumbnail);
  if (has_local_thumbnail_no_remote ||
      // NOTE(julien@vivaldi.com): non-migrated custom thumbnails should not
      // be replaced. We also want them to take over when they do get
      // migrated.
      local_thumbnail_is_non_migrated_custom ||
      // Also do not accept remote urls as thumbnail.
      vivaldi_data_url_utils::IsBookmarkCaptureUrl(remote_thumbnail)) {
    map[vivaldi_bookmark_kit::ThumbnailString()] = local_thumbnail;
  }

  // NOTE(julien@vivaldi.com): Different versions of Vivaldi may have different
  // default thumbnail URLs for the same partner. If we receive a partner
  // bookmark with a default custom thumbnail, we replace it with the current
  // default. Also, a missing thumbnail shouldn't be possible for a partner
  // thumbnail so we replace that as well.
  if (vivaldi_partner.is_valid() &&
      vivaldi_data_url_utils::IsResourceURL(remote_thumbnail) &&
      !local_thumbnail_is_non_migrated_custom) {
    const std::string& partner_thumbnail =
        vivaldi_partners::GetThumbnailUrl(vivaldi_partner);
    if (!partner_thumbnail.empty()) {
      DLOG(INFO) << "UpdateBookmarkNodeFromSpecifics updating to partner "
                   "thumbnail from : \n"
                << map[vivaldi_bookmark_kit::ThumbnailString()] << " to  "
                << partner_thumbnail;
      map[vivaldi_bookmark_kit::ThumbnailString()] = partner_thumbnail;
    }
  }
  // End Vivaldi
...
std::optional<std::string> VivaldiGetSyncedThumbnailChecksumFromSpecifics(
  ...
  for (const sync_pb::MetaInfo& meta_info : specifics.meta_info()) {
    if (meta_info.key() == vivaldi_bookmark_kit::ThumbnailString()) {
      return vivaldi_data_url_utils::GetSyncedStoreChecksumForUrl(
          meta_info.value());
    }
  }
  return std::nullopt;
...
std::optional<std::string> VivaldiGetSyncedThumbnailChecksumFromNode(
  ...
  return vivaldi_data_url_utils::GetSyncedStoreChecksumForUrl(thumbnail);
}
...
bool ShouldReuploadBookmarkForThumbnail(
  ...
  return node->GetMetaInfo(vivaldi_bookmark_kit::ThumbnailString(),
                           &thumbnail) &&
         !vivaldi_data_url_utils::IsLocalPathUrl(thumbnail);
}
