...
void MergeIntoEngineData(const TemplateURL* original_turl,
  ...
  url_to_update->vivaldi_position = original_turl->data().vivaldi_position;
}
    ...
    if ((template_url->safe_for_autoreplace()) &&
        (!default_search_provider[0] ||
         (template_url->prepopulate_id() !=
          default_search_provider[0]->prepopulate_id()) ||
         (template_url->keyword() != default_search_provider[0]->keyword()))) {
...
void GetSearchProvidersUsingLoadedEngines(
  ...
  if (*resource_starter_pack_version < starter_pack_data_version) {
    MergeEnginesFromStarterPackData(service, template_urls,
                                    default_search_provider,
                                    removed_keyword_guids);
