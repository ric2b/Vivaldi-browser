...
struct PrepopulatedPerLanguage {
  ...
  RAW_PTR_EXCLUSION const EngineAndTier* engines;
  size_t size;
};
...
// Vivaldi
raw_ptr<PrepopulatedEngine const> default_search;
#if defined(OEM_POLESTAR_BUILD)
  default_search = &google;
#elif defined(OEM_LYNKCO_BUILD)
  default_search = &ecosia_us;
  // Find the country specific template for Ecosia.
  for (size_t i = 0; i < num_engines; i++) {
    if (wcscmp(L"Ecosia", engines[i].search_engine->name) == 0) {
        default_search = engines[i].search_engine;
        break;
    }
  }
#else
  default_search = engines[0].search_engine;
#endif
    ...
    if (default_search_provider_index) {
      switch (search_type) {
        case SearchType::kMain:
          if (engines[i].search_engine == default_search)
            *default_search_provider_index = i - 2;
          break;
        case SearchType::kPrivate:
          if (engines[i].search_engine == engines[1].search_engine)
            *default_search_provider_index = i - 2;
          break;
        case SearchType::kImage:
          if (engines[i].search_engine->image_url &&
              (engines[i].search_engine == default_search ||
               !found_image_search)) {
            *default_search_provider_index = i - 2;
...
GetPrepopulatedEnginesForEeaRegionCountries(int country_id,
  ...
  const std::vector<EngineAndTier> country_engines =
      GetPrepopulationSetFromCountryID(country_id, GetLangFromPrefs(prefs), nullptr, SearchType::kMain);
...
std::vector<std::unique_ptr<TemplateURLData>> GetPrepopulatedTemplateURLData(
  ...
  if (search_engines::IsEeaChoiceCountry(country_id) && !vivaldi::IsVivaldiRunning() &&
      search_engines::IsChoiceScreenFlagEnabled(
          search_engines::ChoicePromo::kAny)) {
    CHECK(prefs);
  ...
  std::vector<EngineAndTier> engines =
      GetPrepopulationSetFromCountryID(country_id, application_locale, default_search_provider_index, search_type);
...
std::vector<std::unique_ptr<TemplateURLData>> GetPrepopulatedEngines(
  ...
  if (t_urls.empty()) {
    ...
    t_urls = GetPrepopulatedTemplateURLData(country_id, prefs, GetLangFromPrefs(prefs), default_search_provider_index, search_type);
...
std::vector<std::unique_ptr<TemplateURLData>> GetLocalPrepopulatedEngines(
  ...
  return GetPrepopulatedTemplateURLData(country_id, &prefs, application_locale, nullptr, SearchType::kMain);
}
...
std::unique_ptr<TemplateURLData> GetPrepopulatedEngineFromFullList(
  ...
  for (const EngineAndTier& engine_and_tier :
       GetPrepopulationSetFromCountryID(country,GetLangFromPrefs(prefs), nullptr, SearchType::kMain)) {
...
std::unique_ptr<TemplateURLData> GetPrepopulatedDefaultSearch(
    PrefService* prefs,
    search_engines::SearchEngineChoiceService* search_engine_choice_service, SearchType search_type) {
  ...
  std::vector<std::unique_ptr<TemplateURLData>> loaded_urls =
      GetPrepopulatedEngines(prefs, search_engine_choice_service,
                             &default_search_index, search_type);
