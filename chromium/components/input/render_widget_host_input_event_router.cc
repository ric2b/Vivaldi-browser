...
void RenderWidgetHostInputEventRouter::OnRenderWidgetHostViewInputDestroyed(
  ...
  if (view == last_mouse_move_target_) {
    ...
    if (view != last_mouse_move_root_view_) {
      ...
      // NOTE(andre@vivaldi.com) : In case the parent dies we need to get
      // informed. This could happen if a webview was tiled and the user moved
      // the mouse over the closing view. |last_mouse_move_target_| would then
      // be pointing to a deleted object.
      if (last_mouse_move_target_) {
        // It might already be an observed view.
        last_mouse_move_target_->RemoveObserver(this);
        last_mouse_move_target_->AddObserver(this);
      }
...
RenderWidgetTargetResult RenderWidgetHostInputEventRouter::FindMouseEventTarget(
  ...
  if (!target && root_view->IsPointerLocked() &&
      ...
      root_view->GetViewRenderInputRouter()->delegate() &&
      // Vivaldi GUI web_contents may/will not have PointerLock widget
      // (VB-68886)
...
#if !BUILDFLAG(IS_IOS)
  if (vivaldi::IsVivaldiRunning() &&
      blink::WebInputEvent::IsPinchGestureEventType(event.GetType())) {
    gfx::PointF transformed_point;
    RenderWidgetTargetResult target =
        FindViewAtLocation(root_view, event.PositionInWidget(),
                          viz::EventSource::MOUSE, &transformed_point);

    if (target.view && target.view->GetParentViewInput() &&
        target.view->GetParentViewInput() != root_view) {
      while (target.view->GetParentViewInput() != root_view) {
        // Find the top most child view
        target.view = target.view->GetParentViewInput();
      }
      return {target.view, false, event.PositionInWidget(), true};
    }
  }
#endif
#endif  // !IS_ANDROID
