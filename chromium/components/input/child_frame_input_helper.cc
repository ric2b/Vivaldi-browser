...
blink::mojom::InputEventResultState ChildFrameInputHelper::FilterInputEvent(
  ...
  if (blink::WebInputEvent::IsPinchGestureEventType(input_event.GetType())) {
    ...
    // Both tests were added with ch68. Guest view testing removed with ch132
    // as it seems no longer required.
    if (vivaldi::IsVivaldiRunning() &&
...
#if !BUILDFLAG(IS_ANDROID) && !BUILDFLAG(IS_IOS)
    if (vivaldi::IsVivaldiRunning()) {
      // NOTE(espen@vivaldi.com) Added for ch132. In a guest view setup we do
      // have a consumer and we loose proper overscroll behavior if we do not
      // override. Do not return here - that ensures any code added in the
      // future after this section will not be lost.
    } else
...
void ChildFrameInputHelper::GestureEventAckHelper(
  ...
  // away in order to properly activate mac history navigation.
  if (vivaldi::IsVivaldiRunning()) {
    view_->RenderWidgetHostViewInput::VivaldiForwardTouchpadZoomEventIfNecessary(
        event, ack_result);
  }
