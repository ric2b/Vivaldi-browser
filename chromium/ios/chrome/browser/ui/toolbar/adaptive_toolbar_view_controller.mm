...
- (void)viewDidLoad {
  ...
  // Note:(prio@vivaldi.com) - Prevents address bar color glitch for tablets
  // when app opened from killed state. Nil value on this considers the value is
  // false which shows accent color on the address bar even when tab bar is
  // enabled.
  self.isTabBarEnabled = [VivaldiGlobalHelpers isDeviceTablet];
  // End Vivaldi
