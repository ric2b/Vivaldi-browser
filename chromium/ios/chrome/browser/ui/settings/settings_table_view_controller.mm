...
@interface SettingsTableViewController () <
  ...
  // Coordinator for share feedback view
  VivaldiFeedbackViewCoordinator* _shareFeedbackCoordinator;
  // End Vivaldi
}
...
- (void)tableView:(UITableView*)tableView
  ...
  switch (itemType) {
    ...
    case SettingsItemTypeAppIconSettings:
      ...
      break;
    case SettingsItemTypeVivaldiShareFeedbackSettings: {
      [self showShareFeedbackView];
      break;
    }
    // End Vivaldi
...
- (void)addVivaldiAppearanceSettingItem {
  TableViewModel<TableViewItem*>* model = self.tableViewModel;
  [model addItem:[self vivaldiAppearanceItem]
      toSectionWithIdentifier:SettingsSectionIdentifierVivaldiAppearance];
}

- (TableViewItem*)vivaldiAppearanceItem {
  TableViewDetailIconItem* item = [[TableViewDetailIconItem alloc]
      initWithType:SettingsItemTypeAppearanceSettings];
  item.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
  item.text = l10n_util::GetNSString(IDS_VIVALDI_IOS_APPEARANCE_SETTING_TITLE);
  item.iconImage = [UIImage imageNamed:vAppearanceSetting];
  return item;
}
...
- (TableViewItem*)vivaldiShareFeedbackDetailItem {
  UIColor* tintColor = [UIColor colorNamed:vToolbarButtonColor];
  UIImage* buttonIcon =
      [[UIImage imageNamed:vShareFeedbackSetting]
          imageWithTintColor:tintColor
               renderingMode:UIImageRenderingModeAlwaysOriginal];
  TableViewDetailIconItem* item = [[TableViewDetailIconItem alloc]
      initWithType:SettingsItemTypeVivaldiShareFeedbackSettings];
  item.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
  item.text = l10n_util::GetNSString(IDS_IOS_FEEDBACK_MENU_TITLE);
  item.iconImage = buttonIcon;
  return item;
}

- (void)showShareFeedbackView {
  _shareFeedbackCoordinator =
      [[VivaldiFeedbackViewCoordinator alloc]
         initWithBaseNavigationController:self.navigationController
                                  browser:_browser
                               entryPoint:VivaldiFeedbackViewEntryPointLanding
                             allowsCancel:NO];
  _shareFeedbackCoordinator.delegate = self;
  [_shareFeedbackCoordinator start];
}

- (void)stopVivaldiFeedbackPrompt {
  if (_shareFeedbackCoordinator) {
    [_shareFeedbackCoordinator stop];
    _shareFeedbackCoordinator = nil;
  }
}
