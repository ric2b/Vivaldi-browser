...
- (void)createViewController {
  ...
  // Vivaldi
  _viewController.browserState = self.browser->GetBrowserState();
  // End Vivaldi
...
- (void)closeCurrentTab {
  ...
  if (IsVivaldiRunning())
    canShowTabStrip = [self.viewController canShowTabStrip]; // End Vivaldi
...
- (void)showWhatsNew {
  if (IsVivaldiRunning()) {
    if (!WasVivaldiWhatsNewShown()) {
      [self.viewController openWhatsNewTab];
      setVivaldiWhatsNewShown(
...
- (void)showWhatsNewIPH {
  ...
  // Vivaldi: Let's not show any chrome whats' new promo.
  if (IsVivaldiRunning())
    return; // End Vivaldi
...
- (UIEdgeInsets)snapshotGenerator:(SnapshotGenerator*)snapshotGenerator
  ...
  if (IsVivaldiRunning())
    return [self vivaldiSnapshotGenerator:snapshotGenerator
                snapshotEdgeInsetsForWebState:webState]; // End Vivaldi
...
- (BOOL)shouldAllowOverscrollActionsForOverscrollActionsController:
  ...
  if (IsVivaldiRunning())
    return !_toolbarAccessoryPresenter.presenting; // End Vivaldi
...
- (CGFloat)headerHeightForOverscrollActionsController:
  ...
  // Note: (prio@vivaldi.com): Chrome doesn't show pull to refresh options for
  // tablet. But we do. So, return the primary toolbar height with inset for
  // OverscrollContainerView height when desktop tab is enabled and for tablet.
  // This allows a similar pull to refresh as desktop tab disabled state.
  if (IsVivaldiRunning())
    return self.viewController.headerHeightForOverscroll;
  // End Vivaldi
...
- (UIEdgeInsets)vivaldiSnapshotGenerator:(SnapshotGenerator*)snapshotGenerator
           snapshotEdgeInsetsForWebState:(web::WebState*)webState {
  DCHECK(webState);
  ...
  BOOL canShowTabStrip = [self.viewController canShowTabStrip];
  BOOL bottomOmniboxEnabled = [self.viewController isBottomOmniboxEnabled];
  CGFloat bottomPadding =
      [_toolbarCoordinator expandedSecondaryToolbarHeight] +
          [VivaldiGlobalHelpers safeAreaInsets].bottom;

  UIEdgeInsets insets =
      (bottomOmniboxEnabled && canShowTabStrip) ?
          UIEdgeInsetsMake(0, 0, -bottomPadding, 0) : UIEdgeInsetsZero;
  return _fullscreenController->ResizesScrollView() ? insets
                                                    : maxViewportInsets;
}

#pragma mark - ActivityServiceCommands (Vivaldi)
- (void)showSiteInfo {
  CommandDispatcher* dispatcher = self.browser->GetCommandDispatcher();
  [HandlerForProtocol(dispatcher, PageInfoCommands) showPageInfo];
}
