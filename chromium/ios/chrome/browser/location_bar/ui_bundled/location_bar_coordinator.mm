...
- (void)updateAfterNavigatingToNTP {
  ...
  // Vivaldi
  [self.viewController setLoadingState:NO];
...
- (BOOL)isPageLoading {
  ...
  return (currentWebState) ? currentWebState->IsLoading() : NO;
}
...
- (void)setTrackerBlockerSettingForActiveWebState:(ATBSettingType)setting {
  self.viewController.atbSettingForActiveWebState = setting;
}
...
- (void)focusOmniboxWithoutAutocompletePopup {
  [self.omniboxCoordinator focusOmniboxWithoutAutocompletePopup];
}
