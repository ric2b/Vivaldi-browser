  ...
  return {
    ...
    resolveId(source, originScript) {
      ...
      if (source.startsWith('//')) {
        ...
        // vivaldi
        const chromiumSourcePath = path.join("chromium", sourcePath)
        if (chromiumSourcePath in pathMappings) {
          return pathMappings[chromiumSourcePath]
        }
      ...
      if (!source.startsWith('/')) {
        ...
        // vivaldi
        if (source.startsWith('./text-fragment-utils.js')) {
          return fullPath;
        }
