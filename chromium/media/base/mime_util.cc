...
#if defined(USE_SYSTEM_PROPRIETARY_CODECS)
  void InitializeProprietaryMimeTypes();
...
#if defined(USE_SYSTEM_PROPRIETARY_CODECS)
// List of proprietary types only supported by Opera.
static const char* const ipc_media_pipeline_types[] = {
  "audio/3gpp",
  "audio/3gpp2",
...
void MimeUtil::InitializeMimeTypeMaps() {
  ...
  InitializeProprietaryMimeTypes();
#endif
...
#if defined(USE_PROPRIETARY_CODECS) && defined(USE_SYSTEM_PROPRIETARY_CODECS)
void MimeUtil::InitializeProprietaryMimeTypes() {
#if defined(OS_MACOSX)
  allow_proprietary_codecs_ = true;
  for (size_t i = 0; i < arraysize(ipc_media_pipeline_types); ++i) {
    if (base::StartsWithASCII(ipc_media_pipeline_types[i], "audio/", true))
      media_map_.insert(ipc_media_pipeline_types[i]);
  }
#endif  // defined(OS_MACOSX)
