...
void OperaTestFile(MediaContainerName expected,
  ...
  EXPECT_EQ(
      expected,
      OperaDetermineContainer(reinterpret_cast<const uint8*>(buffer), read))
...
TEST(ContainerNamesTest, OperaFileCheckMP4) {
  ...
  EXPECT_EQ(
      CONTAINER_H264,
      OperaDetermineContainer(reinterpret_cast<const uint8*>(kBug263073Buffer),
                              sizeof(kBug263073Buffer)));
}

uint8 kBigBoxSize[] = {0x00, 0x66, 0x00, 0x18, 0x66, 0x74, 0x79,
                       0x70, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                       0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                       0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00};

uint8 kNegativeBoxSize[] = {0xF2, 0x26, 0x01, 0x1C, 0x66, 0x74, 0x79,
                            0x70, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                            0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00};
...
TEST(ContainerNamesTest, OperaFileCheckUNKNOWN) {
  ...
  EXPECT_EQ(CONTAINER_UNKNOWN,
            OperaDetermineContainer(reinterpret_cast<const uint8*>(kBigBoxSize),
                                    sizeof(kBigBoxSize)));
  EXPECT_EQ(
      CONTAINER_UNKNOWN,
      OperaDetermineContainer(reinterpret_cast<const uint8*>(kNegativeBoxSize),
