...
WebMediaPlayerImpl::~WebMediaPlayerImpl() {
  ...
  if (ipc_media_pipeline_host_) {
    // This is deleted before the host got a reply from GPU process.
    media_task_runner_->DeleteSoon(FROM_HERE,
...
std::unique_ptr<Renderer> WebMediaPlayerImpl::CreateRenderer(
...
#if defined(USE_SYSTEM_PROPRIETARY_CODECS)
  if (ipc_demuxer_) {
    use_platform_media_pipeline = true;
  }
#endif
  ...
  return renderer_factory_selector_->GetCurrentFactory()->CreateRenderer(
      ...
      client_->TargetColorSpace(), use_platform_media_pipeline);
}
...
void WebMediaPlayerImpl::StartIPCPipeline(std::string mime_type) {
  ...
  std::string adjusted_mime_type = IPCDemuxer::CanPlayType(mime_type, url);
  if (!adjusted_mime_type.empty()) {
    ipc_media_pipeline_host_ = IPCFactory::CreateHostOnMainThread();
    if (ipc_media_pipeline_host_) {
      // Check if the platform can really play the media before constructing
      // the demuxer. This requires to hop to the media thread and back.
      ipc_media_pipeline_host_->init_data_source(data_source_.get());
      base::OnceClosure task = base::BindOnce(
          &IPCMediaPipelineHost::Initialize,
          base::Unretained(ipc_media_pipeline_host_.get()),
          std::move(adjusted_mime_type),
          BindToCurrentLoop(base::BindOnce(
  ...
  if (media::CoreAudioDemuxer::IsSupported(mime_type, url)) {
    auto demuxer = std::make_unique<CoreAudioDemuxer>(data_source_.get());
    audio_demuxer_ = demuxer.get();
    demuxer_ = std::move(demuxer);
  }
...
void WebMediaPlayerImpl::OnIPCMediaHostInitialized(bool success) {
  if (!success) {
    ...
    std::unique_ptr<IPCDemuxer> ipc_demuxer = std::make_unique<IPCDemuxer>(
        std::move(ipc_media_pipeline_host_), media_log_.get());
    ipc_demuxer_ = ipc_demuxer.get();
