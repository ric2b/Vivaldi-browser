...
void WebMediaPlayerImpl::DataSourceInitialized(bool success) {
  ...
  if (mb_data_source_ &&
    ...
    // are not allowed.
    main_task_runner_->PostTask(
        FROM_HERE, base::BindOnce(&WebMediaPlayerImpl::SniffProtocol, weak_this_));
  } else
#endif  // USE_SYSTEM_PROPRIETARY_CODECS
  {
    StartPipeline();
...
void WebMediaPlayerImpl::StartPipeline() {
  ...
  if (load_type_ != kLoadTypeMediaSource) {
    ...
    std::unique_ptr<media::IPCMediaPipelineHost> ipc_media_pipeline_host;
    if (use_platform_media_pipeline) {
...
void WebMediaPlayerImpl::SniffProtocol() {
  protocol_sniffer_.SniffProtocol(
      data_source_.get(),
      base::Bind(&WebMediaPlayerImpl::ProtocolSniffed, weak_this_));
}
