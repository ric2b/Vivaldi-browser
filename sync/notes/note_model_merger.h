...
// similar logic to that in NoteModelAssociator::AssociateModels() to be
// used by the NoteModelTypeProcessor().
class NoteModelMerger {
 ...
 private:
  ...
  const vivaldi::NoteNode* UpdateNoteNodeFromSpecificsIncludingGUID(
      const vivaldi::NoteNode* local_node,
      const RemoteTreeNode& remote_node);
  ...
  const vivaldi::NoteNode* FindMatchingLocalNodeByGUID(
