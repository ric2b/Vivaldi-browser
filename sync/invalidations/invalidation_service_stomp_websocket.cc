...
void InvalidationServiceStompWebsocket::ProcessIncoming() {
  while (!incoming_sizes_.empty()) {
    ...
    size_t read_size = std::min(remaining_size, buffer.size());

    incoming_message_.append(
        base::as_string_view(buffer.subspan(0, read_size)));
...
bool InvalidationServiceStompWebsocket::HandleFrame() {
  ...
  if (command == kConnectedCommand) {
    ...
    const auto& session_header = headers.find(kSessionHeader);
    if (session_header == headers.end()) {
      return false;
    }
    ...
    std::string message;
    if (!base::Base64Decode(body.substr(0, body_end), &message)) {
      return false;
    }
    client_->OnInvalidation(message);
  } else {
