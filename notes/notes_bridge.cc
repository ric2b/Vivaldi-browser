...
void NotesBridge::GetAllFoldersWithDepths(
  ...
  while (!stk.empty()) {
    ...
    stk.pop();
    Java_NotesBridge_addToNoteIdListWithDepth(
        env, j_folders_obj, node->id(), GetNoteType(node), j_depths_obj, depth);
    noteList.clear();
...
ScopedJavaLocalRef<jobject> NotesBridge::GetRootFolderId(
    ...
    const JavaParamRef<jobject>& obj) {
  const NoteNode* root_node = notes_model_->root_node();
  ScopedJavaLocalRef<jobject> folder_id_obj = JavaNoteIdCreateNoteId(
...
ScopedJavaLocalRef<jobject> NotesBridge::GetMainFolderId(
  ...
  ScopedJavaLocalRef<jobject> folder_id_obj =
      JavaNoteIdCreateNoteId(env, main_node->id(), GetNoteType(main_node));
  return folder_id_obj;
}
...
ScopedJavaLocalRef<jobject> NotesBridge::GetTrashFolderId(
  ...
  ScopedJavaLocalRef<jobject> folder_id_obj =
      JavaNoteIdCreateNoteId(env, trash_node->id(), GetNoteType(trash_node));
  return folder_id_obj;
}
...
ScopedJavaLocalRef<jobject> NotesBridge::GetOtherFolderId(
  ...
  ScopedJavaLocalRef<jobject> folder_id_obj =
      JavaNoteIdCreateNoteId(env, other_node->id(), GetNoteType(other_node));
  return folder_id_obj;
}
...
void NotesBridge::SetNoteUrl(JNIEnv* env,
  ...
  DCHECK(IsLoaded());
  notes_model_->SetURL(GetNodeByID(id, type),
...
void NotesBridge::GetNotesForFolder(
  ...
  const NoteNode* folder = GetFolderWithFallback(folder_id, type);
  ...
  ScopedJavaLocalRef<jobject> folder_id_obj =
      JavaNoteIdCreateNoteId(env, folder->id(), GetNoteType(folder));
...
void NotesBridge::SearchNotes(JNIEnv* env,
  ...
  for (const std::pair<int, NoteNode::Type>& node : results) {
    Java_NotesBridge_addToNoteIdList(env, j_list, node.first, node.second);
  }
}
...
ScopedJavaLocalRef<jobject> NotesBridge::AddFolder(
  ...
  const NoteNode* parent = GetNodeByID(note_id, type);
  ...
  ScopedJavaLocalRef<jobject> new_java_obj =
      JavaNoteIdCreateNoteId(env, new_node->id(), GetNoteType(new_node));
  return new_java_obj;
}
...
void NotesBridge::DeleteNote(JNIEnv* env,
  ...
  const NoteNode* node = GetNodeByID(note_id, type);
  if (!IsEditable(node)) {
    NOTREACHED();
    return;
  }
...
void NotesBridge::MoveNote(JNIEnv* env,
  ...
  const NoteNode* new_parent_node = GetNodeByID(note_id, type);
...
ScopedJavaLocalRef<jobject> NotesBridge::CreateJavaNote(const NoteNode* node) {
  ...
  return Java_NotesBridge_createNoteItem(
      ...
      /*parent == nullptr ? 0 : */ GetNoteType(parent), IsEditable(node),
      IsManaged(node));
}
...
const NoteNode* NotesBridge::GetFolderWithFallback(long folder_id, int type) {
  const NoteNode* folder = GetNodeByID(folder_id, type);
...
bool NotesBridge::IsEditable(const NoteNode* node) const {
  if (!node ||
      (node->type() != NoteNode::FOLDER && node->type() != NoteNode::NOTE &&
       node->type() != NoteNode::SEPARATOR)) {
    return false;
  }
...
int NotesBridge::GetNoteType(const NoteNode* node) {
