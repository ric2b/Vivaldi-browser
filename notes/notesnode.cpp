...
Notes_Node::Notes_Node(int64 id) :
  type_(NOTE),
  id_(id),
  creation_time_(base::Time::Now()) // This will be overwritten if read from file.
{
}
...
base::Value *Notes_Node::WriteJSON() const
{
  ...
  value->SetString("type", (type_ == FOLDER ? "folder" : "note"));
  if (type_ == FOLDER)
  {
  ...
  else
  {
    ...
    value->SetString("date_added", base::Int64ToString(creation_time_.ToInternalValue()));
...
bool Notes_Node::SaveNote()
{
  //TODO
  return false;
}

bool Notes_Node::LoadNote()
{
  //TODO
  return false;
}
