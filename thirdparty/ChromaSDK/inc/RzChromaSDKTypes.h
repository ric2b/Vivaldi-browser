...
typedef DWORD RZCOLOR;  //!< Color data. 1st byte = Red; 2nd byte = Green; 3rd
                        //!< byte = Blue; 4th byte = Alpha (if applicable)
...
typedef enum EFFECT_TYPE {
  ...
  CHROMA_WAVE,  //!< Wave effect (This effect type has deprecated and should not
                //!< be used).
  CHROMA_SPECTRUMCYCLING,  //!< Spectrum cycling effect (This effect type has
                           //!< deprecated and should not be used).
  CHROMA_BREATHING,  //!< Breathing effect (This effect type has deprecated and
                     //!< should not be used).
  CHROMA_BLINKING,   //!< Blinking effect (This effect type has deprecated and
                     //!< should not be used).
  CHROMA_REACTIVE,   //!< Reactive effect (This effect type has deprecated and
                     //!< should not be used).
  CHROMA_STATIC,     //!< Static effect.
  CHROMA_CUSTOM,     //!< Custom effect. For mice, please see
                     //!< Mouse::CHROMA_CUSTOM2.
...
//! Spectrum cycling effect (This effect type has deprecated and should not be
//! used).
typedef struct SPECTRUMCYCLING_EFFECT_TYPE {
  RZSIZE Size;  //!< Size of the structure. Size =
                //!< sizeof(SPECTRUMCYCLING_EFFECT_TYPE)
  DWORD Param;  //!< Extra parameters.
...
typedef enum EFFECT_TYPE {
  CHROMA_NONE = 0,   //!< No effect.
  CHROMA_BREATHING,  //!< Breathing effect (This effect has deprecated and
                     //!< should not be used).
  CHROMA_CUSTOM,     //!< Custom effect.
  CHROMA_REACTIVE,   //!< Reactive effect (This effect has deprecated and should
                     //!< not be used).
  CHROMA_STATIC,     //!< Static effect.
  CHROMA_SPECTRUMCYCLING,  //!< Spectrum cycling effect (This effect has
                           //!< deprecated and should not be used).
  CHROMA_WAVE,  //!< Wave effect (This effect has deprecated and should not be
                //!< used).
  CHROMA_RESERVED,    //!< Reserved.
...
typedef struct CUSTOM_KEY_EFFECT_TYPE {
  COLORREF Color[MAX_ROW][MAX_COLUMN];  //!< Grid layout. 6 rows by 22 columns.
  COLORREF Key[MAX_ROW][MAX_COLUMN];    //!< Keys information. 6 rows by 22
                                        //!< columns. To indidate there is a key
                                        //!< effect, OR with 0x01000000. i.e.
                                        //!< Key[0][1] = 0x01000000 | Color;
...
//! Mice
namespace Mouse {
//! Maximum number of custom LEDs (old definition to maintain backward
//! compatibility).
...
//! Mouse LED Id defintion (This effect type has deprecated and should not be
//! used).
typedef enum RZLED {
  RZLED_NONE = 0,         //!< No LED.
  ...
  RZLED_SIDE_STRIP1 =
      4,  //!< Side strip LED 1. (For Mamba TE, starts from top left hand)
...
typedef enum EFFECT_TYPE {
  CHROMA_NONE = 0,   //!< No effect.
  CHROMA_BLINKING,   //!< Blinking effect (This effect has deprecated and should
                     //!< not be used).
  CHROMA_BREATHING,  //!< Breathing effect (This effect has deprecated and
                     //!< should not be used).
  CHROMA_CUSTOM,     //!< Custom effect (old definition to maintain backward
                     //!< compatibility).
  CHROMA_REACTIVE,   //!< Reactive effect (This effect has deprecated and should
                     //!< not be used).
  CHROMA_SPECTRUMCYCLING,  //!< Spectrum cycling effect (This effect has
...
//! Indexes of the LED are defined in RZLED2.i.e. Row =
//! HIBYTE(RZLED2_SCROLLWHEEL), Column = LOBYTE(RZLED2_SCROLLWHEEL)
...
//! Custom effect type.
typedef struct CUSTOM_EFFECT_TYPE {
  RZCOLOR Color[MAX_LEDS];  //!< An array of colors for all the sides of the
                            //!< mousepad. First LED starts from top-right
                            //!< corner. LED 0-4 right side, 5-9 bottom side,
                            //!< 10-14 left side.
} CUSTOM_EFFECT_TYPE;
...
typedef struct CUSTOM_EFFECT_TYPE {
  RZCOLOR Color[MAX_ROW][MAX_COLUMN];  //!< Custom effect.
                                       //!< For Razer Tartarus Chroma only
                                       //!< Color[0] is valid. Use index '0' to
                                       //!< change the keypad color.
} CUSTOM_EFFECT_TYPE;
