...
std::optional<PathType> ParsePath(base::StringPiece path, std::string* data) {
...
// Parse the full url.
std::optional<PathType> ParseUrl(base::StringPiece url, std::string* data) {
  if (url.empty())
    return std::nullopt;
...
bool IsResourceURL(base::StringPiece url, std::string* resource_path) {
...
std::optional<std::string> GetSyncedStoreChecksumForUrl(
