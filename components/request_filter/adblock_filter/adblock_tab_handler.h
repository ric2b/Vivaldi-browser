...
class TabHandler {
 public:
  ...
  TabHandler(base::Time reporting_start,
             CounterGroup blocked_domains,
             CounterGroup blocked_for_origin,
             base::RepeatingClosure schedule_save);
  ~TabHandler();
  TabHandler(const TabHandler&) = delete;
  TabHandler& operator=(const TabHandler&) = delete;

  base::WeakPtr<TabHandler> AsWeakPtr() { return weak_factory_.GetWeakPtr(); }
 ...
 private:
  ...
  base::WeakPtrFactory<TabHandler> weak_factory_;
};
