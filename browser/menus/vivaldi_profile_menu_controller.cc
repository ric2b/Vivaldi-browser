...
void ProfileMenuController::CollectTargetProfiles(
    Profile* activeProfile, std::vector<ProfileAttributesEntry*>& list) {
...
void ProfileMenuController::Populate(base::string16 label,
  ...
  if (target_profiles_entries.size() == 1) {
    int menu_index = static_cast<int>(
        rv_context_menu_->get_profile_link_paths().size());
    ...
    menu_model->AddItem(command_id,
        l10n_util::GetStringFUTF16(ids.item_string, entry->GetName()));
...
bool ProfileMenuController::IsCommandIdEnabled(int command_id,
    const content::ContextMenuParams& params, bool *enabled) {
  Ids ids(GetIds(is_image_));
  if (command_id == ids.menu ||
      (command_id >= ids.first && command_id <= ids.last)) {
    if (is_image_) {
      // The controller is set up for canvas elements as well.
      *enabled = params.media_type == blink::mojom::ContextMenuDataMediaType::kImage;
    } else {
      *enabled = true;
    }
    return true;
